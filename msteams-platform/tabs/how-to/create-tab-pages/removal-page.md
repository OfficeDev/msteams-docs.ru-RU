...
Когда пользователь выбирает команду **"** Удалить" из раскрывающегося меню вкладки, Teams `removeUrl` загружает необязательную страницу **,** назначенную на странице конфигурации, в iFrame. Пользователю отображается кнопка, `onClick()` `pages.config.setValidityState(true)` загруженная с функцией, которая вызывает и включает кнопку "Удалить", показанную в нижней части iFrame страницы удаления.
