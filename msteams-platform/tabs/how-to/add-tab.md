...
Вы можете использовать приложение App Studio в клиенте Microsoft Teams, чтобы помочь создать манифест приложения. Если у вас нет приложения App Studio, установленного в Teams, ![выберите](/microsoftteams/platform/assets/images/tab-images/storeApp.png) **Приложение Store в** левом нижнем углу приложения Teams и найдите приложение App Studio. Найдя плитку, выберите ее и нажмите кнопку установить в диалоговом окне всплывающее окно.
...
## <a name="create-your-app-package-manually"></a>Создание пакета приложений вручную
...
|`scopes`|Массив перечисления|1 |✔|Статические вкладки поддерживают только `personal` область, что означает, что ее можно подготовить только в составе личного приложения.|

#### <a name="simple-personal-tab-manifest-example"></a>Пример манифеста простой личной вкладки

В приведенном ниже примере показан `staticTabs` только массив из манифеста приложения.
...
Вкладки каналов и групп добавляются в `configurableTabs` массив. Вы можете объявить только одну вкладку "канал/группа `configurableTabs` " в массиве.
...
|`scopes`|Массив перечисления|1 |✔|Настраиваемые вкладки поддерживают только области `team` и `groupchat` области. |

#### <a name="simple-channelgroup-tab-manifest-example"></a>Пример манифеста простого канала/группы вкладок

В приведенном ниже примере показан `configurableTabs` только массив из манифеста приложения.
...
1. Просмотрите вкладку ваш канал/Группа:
    ...
    * Следуйте указаниям по добавлению вкладки. Обратите внимание на то, что существует настраиваемое диалоговое окно настройки для вкладки канал/группа. Нажмите кнопку **сохранить** , и вкладка будет добавлена на панель вкладок канала.
