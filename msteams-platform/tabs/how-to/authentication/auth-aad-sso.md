...
>
> ✔SDK JavaScript Teams (_Версия_: 1.11 и более поздние версии) для работы единого входа на боковой панели собрания.
>
> Для лучшего взаимодействия с Teams используйте последнюю версию iOS и Android.[!NOTE]
> **Быстрый запуск**  
>
...
Для регистрации приложения через портал Azure AD выполните следующие действия.
...
1. На странице **Регистрация приложения** задайте следующие значения параметров.
    ...
    2. В разделе **Поддерживаемые типы учетных записей** выберите тип учетной записи одного клиента или нескольких клиентов. ¹
...
1. В разделе **Управление** выберите **Предоставление API**.
    ...
    >
    > * Используйте буквы нижнего регистра для доменного имени, не используйте верхний регистр. Например, чтобы создать службу приложения или веб-приложение, введите базовое имя ресурса как `demoapplication`. Тогда URL-адрес будет `https://demoapplication.azurewebsites.net`. Но если вы используете базовое имя ресурса в качестве `DemoApplication`, URL-адрес будет `https://DemoApplication.azurewebsites.net`, и это поддерживается в классических приложениях, веб-версиях и iOS, но не в Android.
...
1. Выберите **Добавить область**. В открывшейся панели введите **access_as_user** в качестве параметра **Имя области**.
...
> [!NOTE]
> Чтобы избежать таких ошибок, как `Teams SDK Error: resourceDisabled`, убедитесь, что URI идентификатора приложения правильно настроен в регистрации приложения Azure AD и в приложении Teams.
...
Чтобы запросить дополнительное согласие с помощью API Auth, выполните следующие действия.
