...
keywords: Вкладки проверки подлинности teams Microsoft Azure Active Directory (Azure AD) Graph API Delegated permission access token scope
ms.openlocfilehash: 76b474f69b31d9c9b9925803ee7c0240f9e5a7c4
ms.sourcegitcommit: e16b51a49756e0fe4eaf239898e28d3021f552da
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/04/2022
ms.locfileid: "65888173"
---
...
3. Выберите **+Добавить разрешения для** добавления разрешений API Microsoft Graph.
...
1. Введите сведения о конфигурации платформы.
    ...
    3. Выберите маркеры, которые azure AD будет отправлять для вашего приложения.
...
Вам потребуется получить маркер доступа для Microsoft Graph. Это можно сделать с помощью потока OBO Azure AD.
...
1. Полученный с помощью маркера `getAuthToken()` маркер должен быть обменяться на стороне сервера с помощью потока Azure [AD от](/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow) имени, чтобы получить доступ к другим API Graph. Убедитесь, что для этого обмена вы используете конечную точку Graph v2.
...
3. В случае сбоя обмена необходимо запросить согласие. Используйте пользовательский интерфейс, чтобы попросить пользователя приложения предоставить другое согласие. Этот пользовательский интерфейс должен включать кнопку, которая активирует диалоговое окно согласия Azure AD с использованием [автоматической проверки подлинности](~/concepts/authentication/auth-silent-aad.md).
4. При запросе дополнительного согласия от Azure AD `prompt=consent` необходимо включить параметр [строки](~/tabs/how-to/authentication/auth-silent-aad.md#get-the-user-context) запроса в Azure AD, в противном случае Azure AD не будет запрашивать другие области.
