...
author: laujan
description: Узнайте, как создать личную вкладку. Выберите среду MVC Node.js, ASP.NET Core или ASP.NET Core. Создание приложения, добавление содержимого, создание пакета, сборка и запуск приложения.
...
1. Откройте **tab.ts** в Visual Studio Code из следующего пути, чтобы указать страницу содержимого в iFrame:
...
1. В Visual Studio Обозреватель решений откройте **файл PersonalTab.cshtml** из папки **Pages** и добавьте `microsoftTeams.app.initialize()` в `<script>` теги.
...
1. Имя файла пакета приложения — и `tab.zip` доступно по `/bin/Debug/netcoreapp3.1/tab.zip` пути.
...
1. В разделе **URL-адреса приложений** обновите политику конфиденциальности до `https://<yourngrokurl>/privacy` и условия использования и нажмите **кнопку**`https://<yourngrokurl>/tou` Сохранить.

1. В **разделе Функции приложений** выберите **Личное приложение** > **Создание первой вкладки личного приложения** , введите имя и обновите **URL-адрес содержимого** на `https://<yourngrokurl>/personalTab`. Оставьте поле URL-адрес веб-сайта пустым, выберите **Контекст** как personalTab в раскрывающемся списке и выберите **Подтвердить**.
...
1. В Visual Studio Обозреватель решений откройте **файл PersonalTab.cshtml** из папки **Views** > **PersonalTab** и добавьте `microsoftTeams.app.initialize()` внутри `<script>` тегов.
...
1. В **поле Сведения для разработчика** добавьте необходимые сведения, а в **поле Веб-сайт (должен быть допустимый URL-адрес HTTPS)** укажите URL-адрес HTTPS ngrok.
...
* [Вкладки на мобильных устройствах](../design/tabs-mobile.md)
