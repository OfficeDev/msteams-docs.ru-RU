...
1. Задайте свои значения для ряда вопросов, заданных разработчиком приложений Microsoft Teams для обновления файла`manifest.json`.
    ...
    * **Быстрое формирование шаблонов (Да или нет)**
...
1. Перейдите в Microsoft Teams и выберите **Приложения**&nbsp;:::image type="content" source="~/assets/images/tab-images/store.png" alt-text="Teams Store":::.
...
1. На левой панели Teams выберите многоточие &#x25CF;&#x25CF;&#x25CF; а затем выберите загруженное приложение, чтобы просмотреть личную вкладку.
   ...
   Вы успешно создали и добавили личную вкладку в Teams.
...
Эти файлы должны быть заархивированы в пакете приложения для использования при загрузке вкладки в Teams. Microsoft Teams загружает `contentUrl` указанный в манифесте, встраивает его в <iframe\> и отображает на вкладке.
...
1. В обозревателе решений Visual Studio откройте **PersonalTab.cshtml** из папки **Страницы**, добавьте `microsoftTeams.initialize()` в `<script>` теги и сохраните.
...
1. Имя файла пакета приложения — `tab.zip` и оно доступно по пути `/bin/Debug/netcoreapp3.1/tab.zip`.
...
1. В разделе **Функции приложения**, выберите **Личное приложение** > **Создайте вкладку своего первого личного приложения**, введите имя и обновите **URL-адрес содержимого** с помощью`https://<yourngrokurl>/personalTab`. Оставьте поле URL-адреса веб-сайта пустым и выберите **Context as personalTab** в раскрывающемся списке, затем **Добавить**.
...
Эти файлы должны быть заархивированы в пакете приложения для использования при загрузке вкладки в Teams. Microsoft Teams загружает `contentUrl` указанное в манифесте, встраивает его в IFrame и отображает на вкладке.
...
1. В обозревателе решений Visual Studio откройте **PersonalTab.cshtml** из папки **Представления** > **PersonalTab**, добавьте `microsoftTeams.initialize()` в теги `<script>` и сохраните.
...
Начиная с версии манифеста 1.7, разработчики могут переупорядочивать все вкладки в своем личном приложении. В частности, разработчик может переместить вкладку **чат бота**, которая по умолчанию всегда находится на первой позиции, в любое место в заголовке вкладки личного приложения. Объявлены два зарезервированных `entityId` ключевых слова вкладки, **беседы** и **сведения**.
