...
Для отображения соответствующего контента на вкладке должны потребоваться контекстные сведения:
...
Например, предположим, что в манифесте вкладки вы установите атрибут, чтобы пользователь, вписав его, получил `configURL` `"https://www.contoso.com/config?name={loginHint}&tenant={tid}&group={groupId}&theme={theme}"` следующие атрибуты:

* Имя пользователя — "user@example.com".
* Их ID клиента компании является 'e2653c-etc'.
* Они являются членами группы Office 365 id '00209384-etc'.
* Пользователь установил свою Teams тему на "темную".
...
Переменная контекста выглядит следующим образом:
