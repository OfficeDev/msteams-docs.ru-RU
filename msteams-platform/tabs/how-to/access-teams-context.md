...
* {Ослокалеинфо}: более подробные сведения о языковом стандарте из ОС пользователя, если они доступны. Можно использовать вместе с:
    * пакет @microsoft/ГЛОБЕ NPM, чтобы обеспечить соответствие приложения дате ОС пользователя и
    * Конфигурация формата времени.
* {sessionId}: уникальный идентификатор для текущего сеанса Teams, который будет использоваться для согласования данных телеметрии.
* {Чаннелид}: идентификатор Microsoft Teams для канала, с которым связан контент.
* {Чаннелнаме}: имя канала, с которым связан контент.
* {Чатид}: идентификатор Microsoft Teams для чата, с которой связан контент.
* {URL}: URL-адрес контента этой вкладки.
* {Вебситеурл}: URL-адрес веб-сайта этой вкладки.
* {Фаворитечаннелсонли}: флаг, позволяющий выбирать только избранные каналы.
* {Фаворитетеамсонли}: флаг, позволяющий выбрать только избранные команды.
* {Усертеамроле}: роль текущего пользователя в команде.
* {Теамтипе}: тип команды.
* {Истеамлоккед}: состояние блокировки команды.
* {Истеамарчивед}: архивированный статус команды.
* {Полнотекстовая}: указывает, находится ли вкладка в полноэкранном режиме.
* {Теамситеурл}: корневой сайт SharePoint, связанный с командой.
* {Теамситедомаин}: домен корневого сайта SharePoint, связанного с командой.
* {Теамситепас}: относительный путь к сайту SharePoint, связанному с командой.
* {Чаннелрелативеурл}: относительный путь к папке SharePoint, связанной с каналом.
* {Тенантску}: тип лицензии для текущего клиента пользователей.
* {Рингид}: текущий идентификатор звонка.
* {Аппсессионид}: уникальный идентификатор для текущего сеанса, который используется для согласования данных телеметрии.
* {Комплетионботид}: указывает идентификатор Bot для отправки результата взаимодействия пользователя с модулем задачи.
* {conversationId}: идентификатор беседы.
* {Хостклиенттипе}: тип клиента узла. (Возможные значения: Android, iOS, Web, Desktop и рижел.)
* {Фрамеконтекст}: контекст, в котором загружается URL-адрес вкладки (Content, Task, Setting, Remove, Сидепанел).
* {SharePoint}: этот компонент доступен только при размещении в SharePoint.
* {meetingId}: он используется вкладкой при запуске в контексте собрания.
* {Усерлиценсетипе} Тип лицензии для текущего пользователя.
