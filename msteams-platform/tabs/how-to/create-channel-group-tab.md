...
1. Укажите значения для ряда вопросов, которые генератор приложений Microsoft Teams запрашивает для обновления `manifest.json` файла:
    ...
    * **Имя (компании)? (не более 32 символов)**
        ...
        Название организации можно использовать в манифесте приложения. Введите название компании или выберите **ВВОД** чтобы принять имя по умолчанию.
...
1. В visual Studio Обозреватель решений перейдите в папку **Pages** и откройте **tab.cshtml**.
    ...
    В **файле Tab.cshtml** приложение предоставляет пользователю два варианта отображения вкладки с красным или серым значком. **Триггеры кнопки "Выбрать серый**" или **"** `saveGray()` `saveRed()` Красный" или ,соответственно, задает `pages.config.setValidityState(true)`и включает **сохранение на странице** конфигурации. Этот код сообщает Teams, что вы выполнили настройку требований и можете продолжить установку. Параметры `pages.config.setConfig` заданы. Наконец, `saveEvent.notifySuccess()` вызывается, чтобы указать, что URL-адрес содержимого успешно разрешен.
...
**ChannelGroup.cs** представляет объект сообщения и методы, которые могут вызываться из контроллеров во время настройки.

#### <a name="views"></a>Представления

Это различные представления в ASP.NET Core MVC:

* Домашняя страница: ASP.NET Core рассматривает файлы с именем **Index** как домашнюю страницу сайта по умолчанию. Если URL-адрес браузера указывает на корень сайта, **index.cshtml** может отображаться как домашняя страница приложения.

* Общий доступ. Разметка **частичного представления _Layout.cshtml** содержит общую структуру страницы приложения и общие визуальные элементы, которые также ссылаются на библиотеку Teams.
