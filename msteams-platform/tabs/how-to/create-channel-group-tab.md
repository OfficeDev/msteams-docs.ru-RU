...
> [!IMPORTANT]
> Компонент пути **yourDefaultTabNameTab** — это значение, введенное в генераторе для имени  вкладки по умолчанию и слова **TAB**. Например, `DefaultTabName` **MyTab** then **/MyTabTab/**.
...
У вас должен быть пакет приложения для сборки и запуска приложения в Teams. Пакет приложения создается с помощью задачи gulp, `manifest.json` которая проверяет файл и создает ZIP-папку в `./package` каталоге. В командной строке введите следующую команду:
