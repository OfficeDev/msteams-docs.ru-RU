...
После отрисовки адаптивной карты на вкладке она может реагировать на взаимодействие пользователей. Этот ответ обрабатывается запросом `tab/submit` на вызов.
...
С введением вкладки Адаптивная карта меняется реагировать на запрос `task/submit` бота. Если вы используете вкладку Adaptive Card, бот отвечает на запрос на вызов со стандартной вкладкой продолжить ответ и закрывает `task/submit` модуль задач.  Вкладка Адаптивная карта обновляется путем отрисовки нового списка карт, предоставленных на вкладке **продолжить** ответ.
...
`tab/fetch`запросы могут иметь либо **продолжение,** либо **ответ auth.** Когда запрос запускается и получает ответ на вкладку auth, пользователю отображается вход `tab/fetch` на страницу. 
...
1. Щелкните ссылку **Вход**. Вы перенаправляетсяе на URL-адрес проверки подлинности, предоставленный в свойстве органа `value` ответов **auth.**
