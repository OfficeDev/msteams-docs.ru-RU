...
Начиная с версии `2.0.0`, существующий клиентский пакет SDK Teams (`@microsoft/teams-js` или просто `TeamsJS`) был переработан, чтобы запускать приложения Teams в [Outlook и Office](/m365-apps/overview.md), в дополнение к Microsoft Teams. С функциональной точки зрения последняя версия TeamsJS поддерживает весь спектр функций приложений Teams (версии 1.x.x), добавляя при этом дополнительную возможность размещения приложений Teams в Outlook и Office.
...
|**Существующие приложения только для Teams**| По возможности обновите TeamsJS до версии 2.0 (версия 1.12 все еще поддерживается*)  | 1.12 | [Изучите обратную совместимость TeamsJS](#backwards-compatibility) и [обновите TeamsJS до версии 2.0](#updating-to-the-teams-client-sdk-v200) |
...
**Рекомендуется по возможности использовать последнюю версию TeamsJS (версии не ниже 2.0), чтобы воспользоваться преимуществами последних улучшений и поддержки новых функций (даже для приложений, работающих только в Teams). TeamsJS версии 1.12 все еще поддерживается, но новые функции и улучшения в нее добавляться уже не будут.*
...
Если API не поддерживается или выдает ошибку, добавьте программную логику для корректной обработки сбоя или обходное решение. Например:
