...
ms.locfileid: "65110297"
---
# <a name="tabs-link-unfurling-and-stage-view"></a>Развертывание ссылок вкладок и представление "Экран"

Представление "Экран" — это новый компонент пользовательского интерфейса, который позволяет отображать контент, который открыт в Teams в полноэкранном режиме и закреплен в виде вкладки.

## <a name="stage-view"></a>Представление "Экран"

Представление "Экран" — это компонент полноэкранного пользовательского интерфейса, который можно вызвать для отображения веб-содержимого. Существующая служба развертывания ссылок обновлена и теперь используется для преобразования URL-адресов во вкладки с помощью адаптивных карточек и служб чата. Когда пользователь отправляет URL-адрес в чате или канале, URL-адрес развертывается в адаптивную карточку. Пользователь может выбрать **Просмотр** в карточке и закрепить содержимое как вкладку непосредственно в представлении "Экран".

## <a name="advantage-of-stage-view"></a>Преимущество представления "Экран"

Представление "Экран" обеспечивает более удобный интерфейс просмотра содержимого в Teams. Пользователи могут открывать и просматривать содержимое, предоставленное вашим приложением, не покидая контекст, и закреплять содержимое в чате или канале для быстрого доступа в будущем. Это обеспечивает более высокий уровень взаимодействия пользователей с вашим приложением.
...
> [!NOTE]
> В мобильных клиентах Teams вызов представления "Экран" для приложений, распространяемых через [Магазин Teams](/platform/concepts/deploy-and-publish/apps-publish-overview.md) и не имеющих оптимизированного мобильного интерфейса, открывает стандартный веб-браузер устройства. Браузер открывает URL-адрес, указанный в параметре `websiteUrl` объекта `TabInfo`.
...
| `contentUrl` | String | 2048 | Это свойство является URL-адресом (https://), указывающим на пользовательский интерфейс объекта для отображения на холсте Teams. Это поле обязательно для заполнения.|
| `websiteUrl?` | String | 2048 | Это свойство является URL-адресом (https://) для указания при выборе пользователем просмотра в браузере. Это поле обязательно для заполнения.|
