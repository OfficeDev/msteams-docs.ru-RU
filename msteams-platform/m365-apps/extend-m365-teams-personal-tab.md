...
Чтобы запустить Outlook и Office, вашему приложению необходимо будет зависеть от пакета npm (или более `@microsoft/teams-js@2.0.0-beta.1` *поздней бета-версии).* Хотя код с downlevel версиями поддерживается в Outlook и Office, предупреждения об амортизации будут зарегистрированы, и поддержка downlevel версий в Outlook и Office в конечном итоге `@microsoft/teams-js` `@microsoft/teams-js` прекратится.
...
По завершении утилита обновит файл с зависимостью `package.json` TeamsJS SDK Preview (или более поздней версии), а ваши файлы и файлы будут обновлены с `@microsoft/teams-js@2.0.0-beta.1` `*.js/.ts` `*.jsx/.tsx` помощью:
...
Azure Active Directory для личных вкладок работает так же, как в Office и Outlook, как и в [Teams,](/microsoftteams/platform/tabs/how-to/authentication/auth-aad-sso)однако вам потребуется добавить несколько идентификаторов клиентских приложений в регистрацию приложения Azure AD приложения вкладки на портале регистрации приложений клиента. 
...
> [!IMPORTANT]
> Обратитесь к последним обновлениям Microsoft Teams [- Microsoft 365](https://devblogs.microsoft.com/microsoft365dev/) блог разработчика, чтобы проверить, доступна ли поддержка Office.com для Teams личных приложений для тестовых клиентов.
