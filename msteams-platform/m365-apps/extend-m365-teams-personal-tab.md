...
Личные вкладки — это отличный способ расширить возможности Microsoft Teams. Используя личные вкладки, вы можете предоставить пользователю доступ к приложению прямо в Teams, при этом ему не понадобится выходить из интерфейса или снова входить в систему. В этой предварительной версии личные вкладки могут подсвечиваться в других приложениях Microsoft 365. В этом руководстве показано, как создать существующую личную вкладку Teams и обновить ее для запуска в классическом и веб-интерфейсе Outlook и Office, а также в приложении Office для Android.
...
1. В разделе **Авторизованные клиентские приложения** убедитесь, что добавлены все следующие `Client Id` значения:
    ...
    > [!NOTE]
    > Некоторые клиентские приложения Microsoft 365 совместно используют идентификаторы клиентов.
...
После загрузки неопубликованного приложения в Teams ваша личная вкладка будет доступна в Outlook и Office. Необходимо войти с помощью тех же учетных данных, которые использовались для загрузки неопубликованного приложения в Teams. При запуске приложения Office для Android необходимо перезапустить приложение, чтобы использовать приложение личной вкладки из приложения Office.
...
> [!NOTE]
> Перед установкой приложения выполните действия, чтобы установить последнюю бета-версию [приложения Office](prerequisites.md#mobile) и быть частью бета-версии программы.

Чтобы просмотреть приложение, работающее в приложении Office для Android, выполните следующее.

1. Запустите приложение Office и выполните вход с помощью учетной записи клиента разработки. Если приложение Office для Android уже было запущено до загрузки неопубликованного приложения в Teams, необходимо перезапустить его, чтобы увидеть его среди установленных приложений.
1. Щелкните **значок "Приложения** ". Неопубликованное приложение отображается среди установленных приложений.
1. Щелкните значок приложения, чтобы запустить приложение в приложении Office для Android.

:::image type="content" source="images/office-mobile-apps.png" alt-text="Коснитесь параметра &quot;Приложения&quot; на боковой панели приложения Office, чтобы просмотреть установленные личные вкладки.":::
...
При первом запуске локальной отладки в Office или Outlook вам будет предложено войти в учетную запись клиента Microsoft 365 и установить самозаверяющий тестовый сертификат. Вам также будет предложено вручную установить Teams. Выберите **"Установить в Teams"** , чтобы открыть окно браузера и вручную установить приложение. Затем нажмите **кнопку "** Продолжить", чтобы перейти к отладке приложения в Office или Outlook.
...
#### <a name="mobile-debugging"></a>Отладка мобильных устройств

Отладка Набора средств Teams (`F5`) пока не поддерживается в приложении Office для Android. Вот как можно удаленно отладить приложение, работающее в приложении Office для Android:

1. При отладке с помощью физического устройства Android подключите его к компьютеру разработки и включите параметр отладки [USB](https://developer.android.com/studio/debug/dev-options). Эта функция включена по умолчанию в эмуляторе Android.
1. Запустите приложение Office с устройства Android.
1. Откройте профиль **"Me > Settings > Allow debugging**" (Разрешить отладку) и включите параметр **"Включить удаленную отладку"**.

    :::image type="content" source="images/office-android-enable-remote-debugging.png" alt-text="Снимок экрана: включение удаленной отладки":::

1. **Параметры выхода**.
1. Закройте экран профиля.
1. Выберите **"Приложения** " и запустите неопубликованное приложение для запуска в приложении Office.
1. Убедитесь, что устройство Android подключено к компьютеру разработчика. На компьютере разработчика откройте браузер на странице проверки DevTools. Например, перейдите в `edge://inspect/#devices` Microsoft Edge, чтобы отобразить список веб-представлений Android с поддержкой отладки.
1. Найдите URL-адрес `Microsoft Teams Tab` вкладки и выберите **"Проверить** ", чтобы начать отладку приложения с помощью Средств разработки.

    :::image type="content" source="images/office-android-debug.png" alt-text="Снимок экрана: список веб-представлений в devtool":::
