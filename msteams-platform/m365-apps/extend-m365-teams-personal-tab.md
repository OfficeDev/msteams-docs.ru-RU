...
> * Обновите ссылки на пакет SDK TeamsJS.
> * Измените заголовки политики безопасности содержимого.
> * Обновите регистрацию приложения Microsoft Azure Active Directory (Azure AD) для единого входа.
...
Кроме того, вы можете использовать простой единый вход в *приложение hello world* , уже включенное в Microsoft 365 в следующем разделе [краткого руководства](#quickstart) , а затем перейти к [разделу Загрузка неопубликованного приложения в Teams](#sideload-your-app-in-teams) .
...
Здесь вы можете перейти к [разделу Загрузка неопубликованного приложения в Teams](#sideload-your-app-in-teams) и предварительный просмотр приложения в Outlook и Office. (Манифест приложения и вызовы API TeamsJS уже обновлены для Microsoft 365.)

## <a name="update-the-app-manifest"></a>Обновите манифест приложения

Вам потребуется использовать версию `1.13` схемы [манифеста разработчика Teams](../resources/schema/manifest-schema.md), чтобы включить личную вкладку Teams для запуска в Outlook и Office.
...
[Единый вход Azure Active Directory (AD)](../tabs/how-to/authentication/tab-sso-overview.md) для личных вкладок работает так же, как и в Teams. Однако вам потребуется добавить несколько идентификаторов клиентских приложений в Azure AD регистрации приложения вкладки на *портале Регистрация приложений* клиента.
...
Вы можете закрепить приложение для быстрого доступа или найти его во всплывающем меню с многоточием (**...**) среди последних приложений на боковой панели слева. Закрепление приложения в Teams не закрепите его как приложение в Office или Outlook.
...
> [!NOTE]
> При удалении приложения из Teams оно также удаляется из каталогов **Других приложений** в Outlook и Office. Если вы используете пример приложения Teams Toolkit, предоставленный выше.
...
1. Щелкните значок приложения, чтобы запустить приложение в Office в Интернете.
    ...
    :::image type="content" source="images/office-web-more-apps.png" alt-text="Щелкните параметр &quot;Дополнительные приложения&quot; на боковой панели office.com, чтобы просмотреть установленные личные вкладки.":::
...
1. Запустите приложение Office и выполните вход с помощью учетной записи клиента разработки. Если приложение Office для Android уже работало до загрузки неопубликованного приложения в Teams, необходимо перезапустить его, чтобы увидеть его среди установленных приложений.
...
:::image type="content" source="images/toolkit-debug-targets.png" alt-text="Выбор целевых объектов отладки Teams, Outlook и Office в наборе средств Teams":::
...
:::image type="content" source="images/toolkit-dialog-teams-install.png" alt-text="Диалоговое окно набора средств Установки Teams":::
...
1. Найдите с `Microsoft Teams Tab` помощью URL-адреса вкладки и выберите **проверить** , чтобы начать отладку приложения с помощью средств разработки.
    ...
    :::image type="content" source="images/office-android-debug.png" alt-text="Снимок экрана: список веб-представлений в средстве разработки":::
