...
|Стандартный обмен сообщениями HTTP|Ответы появятся в той же цепочке, что и исходное сообщение-запрос, и могут содержать всё, что содержат сообщения Bot Framework, например, текст, изображения, карточки и эмодзи. Несмотря на то что исходящие веб-перехватчики могут использовать карточки, они не могут использовать никакие действия с карточками, кроме `openURL`.|
...
Появится диалоговое окно [Код проверки подлинности сообщений с помощью хэш-функций (HMAC)](https://security.stackexchange.com/questions/20129/how-and-when-do-i-use-hmac/20301). Это маркер безопасности, используемый для проверки подлинности звонков между Teams и назначенной внешней службой.
...
* Пример: у вас есть бизнес-приложение, которое отслеживает все операции создания, чтения, обновления и удаления. Эти операции с записями сотрудников выполняются пользователями канала Teams из отдела кадров в области клиента Office 365.
