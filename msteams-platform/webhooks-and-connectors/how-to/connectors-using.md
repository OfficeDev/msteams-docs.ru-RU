...
Это сообщение создает следующую карточку в канале:
...
Выполните следующие действия, чтобы узнать, как отправить простую карту в соединителю:

1. В Microsoft Teams нажмите **Дополнительные параметры** (**&#8943;**) рядом с названием канала и выберите **Соединители**.
1. Прокрутите список соединителей до пункта **Входящий веб-перехватчик** и нажмите кнопку **Добавить**.
1. Введите имя веб-перехватчика, отправьте изображение, которое следует связать с данными от веб-перехватчика, и нажмите кнопку **Создать**.
1. Скопируйте веб-перехватчик в буфер обмена и сохраните его. Для отправки информации в Microsoft Teams вам потребуется URL-адрес веб-перехватчика.
...
1. В случае успешного выполнения запроса POST команда `curl` должна возвращать простой отклик **1**.
1. Проверьте клиент Microsoft Teams. Вы увидите новую карточку, опубликованную в группе.
...
1. В случае успешного выполнения запроса POST команда `Invoke-RestMethod` должна возвращать простой отклик **1**.
1. Проверьте канал Microsoft Teams, связанный с URL-адресом веб-перехватчика. На канале должна появиться новая карточка.
...
В следующем manifest.jsфайле содержатся основные элементы, необходимые для тестирования и отправки приложения:
...
1. [Настройка настраиваемой веб-страницы](#setting-up-a-custom-incoming-webhook) в Teams.
1. Создайте файл JSON адаптивной карты:
...
1. Проверьте адаптивную карту с помощью Почтальон.
...
Чтобы тестировать соединитель, отправьте его команде, как в любом другом приложении. Вы можете создать пакет .zip с помощью файла манифеста из панели мониторинга разработчика соединителов, которая была изменена в предыдущем разделе и двух файлах значков.

После отправки приложения откройте список соединителей с любого канала. Прокрутите вниз, чтобы просмотреть приложение в разделе **Uploaded:**
...
[Office 365 Соединители — Microsoft Teams](/connectors/teams/)
