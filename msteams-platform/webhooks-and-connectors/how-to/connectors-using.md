...
Действия сообщений включают шесть видимых кнопок на карте. Каждая кнопка определяется в свойстве сообщения с помощью действий, каждая из которых имеет тип ввода, текстовое поле, выбор даты или список нескольких `potentialAction` `ActionCard` вариантов. Каждое `ActionCard` из них имеет связанное действие, например `HttpPOST` .
