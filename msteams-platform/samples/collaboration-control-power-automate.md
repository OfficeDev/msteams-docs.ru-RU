...
1. На домашней странице портал Azure выберите **Azure Active Directory**. В Azure Active Directory выберите раскрывающийся список для **добавления** и **регистрации приложения**.
   ...
   :::image type="content" source="../assets/images/collaboration-control/azure-active-directory-home-portal.png" alt-text="На снимке экрана показан пример добавления новой регистрации приложения":::

   :::image type="content" source="../assets/images/collaboration-control/new-app-registration.png" alt-text="На снимке экрана показан пример добавления регистрации нового приложения":::
...
1. В регистрации приложения задайте имя приложения и добавьте URI веб-перенаправления.`https://global.consent.azure-apim.net/redirect`
   ...
   :::image type="content" source="../assets/images/collaboration-control/register-an-application.png" alt-text="На снимке экрана показан пример регистрации приложения":::

1. В разделе "Неявное предоставление и гибридные потоки" выберите маркеры доступа и маркеры идентификаторов.

   :::image type="content" source="../assets/images/collaboration-control/authorisation-endpoint-tokens.png" alt-text="На снимке экрана показан пример маркеров и маркеров идентификаторов.":::
...
1. Выберите разрешение API в левой области и **выберите "Добавить** разрешение", а затем выполните поиск **динамического разрешения CRM** .
   ...
   :::image type="content" source="../assets/images/collaboration-control/dynamic-crm.png" alt-text="На снимке экрана показан пример добавления разрешения.":::

1. Убедитесь, **что user_impersonation** в разделе "Разрешения" после выбора Dynamics CRM.

   :::image type="content" source="../assets/images/collaboration-control/admin-consent-required.png" alt-text="На снимке экрана показан пример включения флажка user_impersonation":::
...
1. На странице "& сертификатов" добавьте новый секрет клиента и сохраните значение для последующего использования при настройке безопасности соединителя.
   ...
   :::image type="content" source="../assets/images/collaboration-control/copy-new-secret-value.png" alt-text="На снимке экрана показан пример копирования нового значения секрета":::
...
1. В Центре администрирования **выберите "Среда** " в левой области и выберите среду в списке, в который вы хотите добавить приложение соединителя.
   ...
   :::image type="content" source="../assets/images/collaboration-control/power-platform-admin-center.png" alt-text="На снимке экрана показан пример добавления приложения соединителя":::

1. На странице сведений о среде выберите " **Параметры"**.

   :::image type="content" source="../assets/images/collaboration-control/settings-environment.png" alt-text="На снимке экрана показан пример выбора параметров":::
...
1. Выберите свое приложение в поле поиска и нажмите кнопку "Добавить" еще раз.
   ...
   :::image type="content" source="../assets/images/collaboration-control/add-app-aad.png" alt-text="На снимке экрана показан пример добавления приложения из Azure Active Directory":::

После добавления приложения задайте **бизнес-подразделение** **и роли безопасности** для приложения соединителя. Выберите **"Создать** ", и ваше приложение будет в списке. Задав пользователя приложения в среде, можно перейти к настройке настраиваемого соединителя.
...
1. Откройте PowerApps или Power Automate и выберите меню **"Настраиваемые соединители** ". Выберите **"Изменить** " для соединителя совместной работы.
   ...
   :::image type="content" source="../assets/images/collaboration-control/collaboration-connector.png" alt-text="меню настраиваемого соединителя":::
...
1. На вкладке "Безопасность" введите следующие входные данные:
   ...
   * Область: то же, что и выше. Суффикс по умолчанию (`https://org.crm.dynamics.com/.default`).
   ...
   :::image type="content" source="../assets/images/collaboration-control/dynamic-365-instance.png" alt-text="На снимке экрана показан пример экземпляра Dynamic 365.":::

1. Выберите **"Обновить соединитель"** , чтобы сохранить изменения и разрешить потоку устанавливать подключения.

   :::image type="content" source="../assets/images/collaboration-control/custom-connector.png" alt-text="На снимке экрана показан пользовательский соединитель.":::
...
Триггеры и действия предварительно определены с настраиваемыми входными и выходными данными в качестве шага рабочего процесса. Добавление шага рабочего процесса в правильную позицию рабочего процесса с правильной конфигурацией входных и выходных данных, чтобы определить, когда должен быть вызван триггер или действие.
  ...
  :::image type="content" source="../assets/images/collaboration-control/invoke-the-connector.png" alt-text="На снимке экрана показан пример вызова соединителя.":::
...
* **Триггеры**
  ...
  1. При создании сеанса совместной работы.
      ...
      :::image type="content" source="../assets/images/collaboration-control/colab-session-created-preview.png" alt-text="Созданный сеанс совместной работы":::

      **Размах:** Область, которую следует ограничить, какие строки могут активировать поток.
  ...
  1. При создании или изменении задачи
      ...
      :::image type="content" source="../assets/images/collaboration-control/task-created.png" alt-text="На снимке экрана показан пример создания или изменения задачи":::

      По умолчанию задача планировщика триггеров будет отключена и не будет активироваться. Чтобы включить его, администратор клиента должен выполнить следующие действия:

      * Создайте запрос в службу поддержки по пути Power Apps/Collaboration controls/Settings.
      * Запросите включение среды для соединителя совместной работы и укажите URL-адрес среды (предпочтительный) или идентификатор организации.  
      * В запрос на поддержку можно добавить следующий пример текста: "Включить URL-адрес среды: `url` для соединителя совместной работы".
      * Чтобы открыть запрос в службу поддержки, см [. раздел "Справка и поддержка".](/power-platform/admin/get-help-support)
...
* **Действия**
  ...
  1. Начало сеанса совместной работы
      ...
      **Метаданные (дополнительно):** Добавляет метаданные для сеанса совместной работы.
        ...
        * **Тип OData:** Это поле необходимо указать, если заданы другие ключ-значение и должны точно совпадать с #Microsoft.Dynamics.CRM.m365_collaborationmetadata.
        * **Ключ:** Ключ, связанный с атрибутом метаданных.
        * **Ценность:** Значение, связанное с атрибутом метаданных.

  1. Получение сеанса совместной работы

      ::image type="content" source=".. /assets/images/collaboration-control/retrieve-collab-session.png" alt-text="Снимок экрана — пример, показывающий, как получить сеанс совместной работы.":::
  ...
  1. Обновление сеанса совместной работы
      ...
      :::image type="content" source="../assets/images/collaboration-control/update-collab-session.png" alt-text="На снимке экрана показан пример обновления сеанса совместной работы.":::
     ...
     ***Дополнительные параметры:***
      ...
      * **Тип OData:** Это поле должно быть предоставлено, если заданы другие ключ-значение и должны точно соответствовать #Microsoft.Dynamics.CRM.m365_collaborationmetadata.
      * **Ключ:** Ключ, связанный с атрибутом метаданных.
      * **Ценность:** Значение, связанное с атрибутом метаданных.
  ...
  1. Сопоставление карты совместной работы (внешняя)
      ...
      :::image type="content" source="../assets/images/collaboration-control/associate-collab-map.png" alt-text="На снимке экрана показан пример связывания карты совместной работы.":::
     ...
     **Метаданные:** Добавьте метаданные для карты совместной работы.
     * **Тип OData:** Это поле должно быть предоставлено, если заданы другие ключ-значение и должны точно соответствовать #Microsoft.Dynamics.CRM.m365_collaborationmetadata.
     * **Ключ:** Ключ, связанный с атрибутом метаданных.
     * **Ценность:** Значение, связанное с атрибутом метаданных.
  ...
  1. Сопоставление карты совместной работы (внутренняя)
      ...
      :::image type="content" source="../assets/images/collaboration-control/associate-collab-map-internal.png" alt-text="На снимке экрана показан пример, показывающий, как связать внутреннюю карту совместной работы.":::
     ...
     * **Тип OData:** Это поле должно быть указано, если задан другой ключ или значение, которое должно точно соответствовать #Microsoft.Dynamics.CRM.m365_collaborationmetadata
     * **Ключ:** Ключ, связанный с атрибутом метаданных
     * **Ценность:** Значение, связанное с атрибутом метаданных

  1. Обновление карты совместной работы

      :::image type="content" source="../assets/images/collaboration-control/update-collab-map.png" alt-text="На снимке экрана показан пример обновления карты совместной работы.":::
  ...
  1. Получение метаданных совместной работы
      ...
      :::image type="content" source="../assets/images/collaboration-control/get-collab-metadata.png" alt-text="На снимке экрана показан пример получения метаданных совместной работы.":::
  ...
  1. Создание задачи планировщика
      ...
      :::image type="content" source="../assets/images/collaboration-control/create-planner-task.png" alt-text="На снимке экрана показан пример создания задачи планировщика.":::
     ...
     * **Дата начала:** Дата и время начала задачи. Тип Timestamp представляет сведения о времени и дате с использованием формата ISO 8601 (всегда применяется формат UTC). Например, полуночь 1 января 2014 г. в формате UTC — 2014-01-01T00:00:00Z.

  1. Получение задачи Планировщика

      :::image type="content" source="../assets/images/collaboration-control/get-planner-task.png" alt-text="На снимке экрана показан пример задачи получения планировщика.":::
  ...
  1. Обновление задачи планировщика
      ...
      :::image type="content" source="../assets/images/collaboration-control/update-planner-task-preview.png" alt-text="Обновление задачи планировщика":::

     Это действие обновляет запись задачи планировщика с помощью виртуальной таблицы задач Планировщика элементов управления совместной работы

     * **Идентификатор задачи (обязательно):** Уникальный идентификатор задачи.
     * **Назначения:** Объект в формате JSON, представляющий все назначения задачи. Видеть. Тип ресурса plannerAssignments — Microsoft Graph версии 1.0 | Документация Майкрософт  
...
Ниже приведен пример потоков.

1. Получение ответа от форм Майкрософт, создание сеанса совместной работы и связанной задачи.

   :::image type="content" source="../assets/images/collaboration-control/response-submitted.png" alt-text="На снимке экрана показан пример отправки нового ответа.":::

1. Каждый раз, когда создается сеанс совместной работы, запишите сведения и отправьте уведомление по электронной почте.

   :::image type="content" source="../assets/images/collaboration-control/colab-session-created-preview.png" alt-text="На снимке экрана показан пример созданного сеанса совместной работы":::
