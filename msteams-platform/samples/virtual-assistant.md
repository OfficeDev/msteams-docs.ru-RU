...
description: Узнайте, как создать бот-виртуальный помощник для Microsoft Teams с помощью примеров программного кода и фрагментов кода, реализующих такие функции, как адаптивные карточки, обработка прерываний, запросы модулей задач, области приложения для совместной работы и расширения сообщений; использование манифеста навыка; поддержка нескольких языков, проверка утверждений, интеграция LUIS и режим.
ms.localizationpriority: high
ms.topic: how-to
keywords: Боты-виртуальные помощники Teams
ms.openlocfilehash: 1d968c7fb05d80313d664d9290654911d8dc7638
ms.sourcegitcommit: f15bd0e90eafb00e00cf11183b129038de8354af
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/28/2022
ms.locfileid: "65111467"
---
...
Корпорация Майкрософт опубликовала шаблон [Microsoft Visual Studio](https://marketplace.visualstudio.com/items?itemName=BotBuilder.VirtualAssistantTemplate) для создания виртуальных помощников и навыков. С помощью шаблона Visual Studio можно создать виртуального помощника с текстовым интерфейсом и поддержкой ограниченных форматированных карточек с действиями. Мы усовершенствовали базовый шаблон Visual Studio, добавив в него возможности взаимодействия с платформой Microsoft Teams и отличные интерфейсы для взаимодействия с приложениями Teams. В числе возможностей - поддержка адаптивных карточек с расширенными возможностями, модулей задач, командных или групповых чатов и расширений для обмена сообщениями. Дополнительные сведения о расширении виртуального помощника для Microsoft Teams см. в документе: [Руководство по расширению виртуального помощника для взаимодействия с Microsoft Teams](https://microsoft.github.io/botframework-solutions/clients-and-channels/tutorials/enable-teams/1-intro/).
...
Чтобы правильно перенаправлять запросы, виртуальный помощник должен определить нужную модель LUIS и соответствующий навык, связанный с ней. Однако механизм диспетчеризации нельзя использовать для действий с карточками, так как модель LUIS, связанная с навыком, обучена операциям с текстом действия карточек. Текст действия карточки - это набор фиксированных, предопределенных ключевых слов, и пользователь не добавляет туда своих комментариев.
...
В следующем примере показано, как преобразовать шаблон приложения "Резервирование комнаты" в навык виртуального помощника: "Резервирование комнаты" — это приложение Microsoft Teams, позволяющее пользователям быстро находить и резервировать конференц-зал на 30, 60 или 90 минут, начиная с текущего момента. Длительность резервирования по умолчанию — 30 минут. Бот "Резервирование комнаты" действует в области личных или приватных бесед.
...
Ниже приведены разностные изменения, внесенные для преобразования его в навык, присоединенный к виртуальному помощнику. Аналогичные шаги нужны для преобразования любого существующего бота версии 4 в навык.

### <a name="skill-manifest"></a>Манифест навыка

Манифест навыка — это JSON-файл, предоставляющий параметры навыка: конечную точку обмена сообщениями, идентификатор, имя и другие соответствующие метаданные. Этот манифест отличается от манифеста, используемого для загрузки неопубликованного приложения в Microsoft Teams. Виртуальному помощнику нуже путь к этому файлу в качестве входных данных для прикрепления навыка. Мы добавили следующий манифест в папку wwwroot бота.
...
С другой стороны, боту `Book-a-room room` необходимо использовать модель LUIS, чтобы понять эти команды, если они не введены полностью в письменном виде. Пример: `I want to manage my favorite rooms`.
...
Чтобы изменить параметр `languages`, обновите команду botskills следующим образом:

```json
botskills connect --remoteManifest "<url to skill's manifest>" --luisFolder "<path to luisFolder>" --languages "en-us, your_language_culture" --cs
```

Виртуальный помощник использует `SetLocaleMiddleware` для определения текущего языкового стандарта и вызова соответствующей модели диспетчеризации. Действие Bot Framework имеет поле языкового стандарта, используемого этим ПО промежуточного слоя. Вы можете использовать то же самое для своего навыка. Бот "Резервирование комнаты" не использует это ПО промежуточного слоя, а вместо этого получает языковой стандарт из [сущности clientInfo](https://github.com/microsoft/botframework-sdk/blob/master/specs/botframework-activity/botframework-activity.md#clientinfo) действия Bot Framework.

### <a name="claim-validation"></a>Проверка утверждений

Мы добавили [claimsValidator](https://github.com/nebhagat/msteams-virtual-assistant-dotnet/blob/master/msteams-virtual-assistant-dotnet/Authentication/AllowedCallersClaimsValidator.cs), чтобы ограничить число объектов, которые могут вызывать навык. Чтобы разрешить виртуальному помощнику вызывать этот навык, заполните массив `AllowedCallers` на основе `appsettings` с помощью идентификатора приложения этого виртуального помощника.
...
Обновление действия, например обновление карточки через виртуальный помощник пока не поддерживается ([проблема с GitHub](https://github.com/microsoft/botbuilder-dotnet/issues/3686)). Таким образом, мы заменили все вызовы обновления карточек `UpdateActivityAsync` публикацией новых вызовов карт `SendActivityAsync`.
...
Дополнительные сведения см. в [этом](/microsoftteams/platform/samples/virtual-assistant#add-adaptive-cards-to-your-virtual-assistant) разделе данной документации.
...
* **EndOfConversation**: по завершении беседы навык должен отправить действие `endOfConversation`. В зависимости от действия виртуальный помощник заканчивает контекст с определенным навыком и возвращается в корневой контекст виртуального помощника. У бота "Резервирование комнаты" нет четкого состояния завершения беседы. Поэтому мы не отправляли сообщения `endOfConversation` от бота `Book-a-room`, и когда пользователь хочет вернуться к корневому контексту, это можно сделать с помощью команды `start over`.  
* **Обновление карточки**: обновление карточки через виртуальный помощник пока не поддерживается.  
* **Расширения для сообщений**
  ...
  * Настройка расширений для сообщений предназначена не для отдельных команд, а для всего расширения. Это ограничивает конфигурацию для каждого отдельного навыка с помощью виртуального помощника.
