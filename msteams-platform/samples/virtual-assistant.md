...
description: Узнайте, как создать Виртуальный помощник бота для Microsoft Teams с помощью примеров и фрагментов кода с такими функциями, как адаптивные карты; перебои обработки, запросы модулей задач, области совместных приложений и расширения сообщений; использование манифеста навыков; Поддержка нескольких языков, проверка утверждений, интеграция LUIS и режим.
...
Корпорация Майкрософт опубликовала шаблон [Microsoft Visual Studio для](https://marketplace.visualstudio.com/items?itemName=BotBuilder.VirtualAssistantTemplate) создания виртуальных помощников и навыков. С помощью Visual Studio шаблона можно создать Виртуальный помощник, основанный на тексте, с поддержкой ограниченных богатых карт с действиями. Мы усовершенствовали базовый Visual Studio, чтобы включить Microsoft Teams платформы и Teams приложения. Некоторые возможности включают поддержку богатых адаптивных карт, модулей задач, групп или групповых чатов и расширений обмена сообщениями. Дополнительные сведения о продлении Виртуальный помощник Microsoft Teams см. в учебнике[: расширение Виртуальный помощник до Microsoft Teams](https://microsoft.github.io/botframework-solutions/clients-and-channels/tutorials/enable-teams/1-intro/).    
...
Этот недостаток решается путем встраив сведения о навыках в полезной нагрузке действия карты. Каждый навык должен встраить `skillId` в  `value` область действий карт. Необходимо убедиться, что каждое действие карты несет соответствующие сведения о навыках, и Виртуальный помощник использовать эту информацию для отправки.
...
Далее `SkillCardActionData` класс в Виртуальный помощник вводится для извлечения `skillId` из полезной нагрузки действия карты.
...
Аналогичный подход следует для отправки действий по картам и ответов модулей задач. Модуль задач для получения и отправки данных действий обновляется, чтобы включить `skillId`. Метод расширения действий `GetSkillId` извлекает `skillId` из полезной нагрузки, которая содержит сведения о навыке, который необходимо вызвать.
...
После вызова команд пользователем Виртуальный помощник соответствующий навык путем разбора ИД команды, `:<skill_id>` обновления действия путем удаления дополнительного суффикса из командного ИД и его переадэффинга в соответствующий навык. Код для навыка не должен обрабатывать дополнительный суффикс. Таким образом, можно избежать конфликтов между командными ИД между навыками. С помощью этого подхода все команды поиска и действий навыка во всех контекстах, таких как **compose**, **commandBox** и сообщения, будут  работать с помощью Виртуальный помощник.
...
Некоторые действия по расширению обмена сообщениями не включают командный ID. Например, содержит `composeExtension/selectItem` только значение действия касания. Чтобы определить связанный навык, `skillId`  при формировании ответа для каждого элемента присоединяется к каждой карте элемента `OnTeamsMessagingExtensionQueryAsync`. Это похоже на подход к добавлению [адаптивных карт в Виртуальный помощник](#add-adaptive-cards-to-your-virtual-assistant).
...
С помощью этого подхода любая команда, `book room` `manage favorites` `Book-a-room` выдаемая пользователем для Виртуальный помощник или идентифицирована как команда, связанная с ботом, и передается на этот навык.
...
Обновление активности, например обновления карт, пока не поддерживается Виртуальный помощник ([проблема github](https://github.com/microsoft/botbuilder-dotnet/issues/3686)). Таким образом, мы заменили все вызовы обновления карт `UpdateActivityAsync` на размещение новых вызовов карт `SendActivityAsync`.
