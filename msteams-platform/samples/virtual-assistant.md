...
description: Узнайте, как создать Виртуальный помощник бота для Microsoft Teams с помощью примеров кода и фрагментов с такими функциями, как адаптивные карты; обработка перерывов, запросов модулей задач, областей совместных приложений и расширений сообщений; с помощью манифеста навыков; Поддержка нескольких языков, проверка утверждений, интеграция LUIS и режим.
...
ms.locfileid: "63452853"
---
# <a name="create-virtual-assistant"></a>Создание виртуального помощника

Виртуальный помощник является шаблоном Microsoft с открытым исходным кодом, который позволяет создавать надежное решение для беседы, сохраняя полный контроль над пользовательским опытом, организационным брендингом и необходимыми данными. Основной [Виртуальный помощник](https://microsoft.github.io/botframework-solutions/overview/virtual-assistant-template) является основным строительным блоком, который объединяет технологии Майкрософт, необходимые для создания Виртуальный помощник, в том числе [SDK Bot Framework](https://github.com/microsoft/botframework-sdk), [Language Understanding (LUIS)](https://www.luis.ai/) и [QnA Maker](https://www.qnamaker.ai/). Он также объединяет основные возможности, включая регистрацию навыков, связанные учетные записи, основные беседы, чтобы предложить пользователям широкий спектр бесшовных взаимодействий и взаимодействия. Кроме того, возможности шаблона включают в себя богатые примеры многоиспользоваемых разговорных [навыков](https://microsoft.github.io/botframework-solutions/overview/skills).  Отдельные навыки интегрированы в Виртуальный помощник, чтобы включить несколько сценариев. С помощью SDK Bot Framework навыки представлены в форме исходных кодов, что позволяет настраивать и расширять по мере необходимости. Дополнительные сведения о навыках Bot Framework см. в этой части[.](https://microsoft.github.io/botframework-solutions/overview/skills/) В этом документе вы можете Виртуальный помощник реализации для организаций, как создать Teams Виртуальный помощник, связанный пример, пример кода и ограничения Виртуальный помощник.
На следующем изображении отображается обзор виртуального помощника:

![Виртуальный помощник обзор схемы](../assets/images/bots/virtual-assistant/overview.png)

Действия текстовых сообщений отправляются в связанные навыки Виртуальный помощник ядром с помощью [модели отправки](/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&tabs=cs&preserve-view=true).
...
Этот недостаток решается путем встраив сведения о навыках в полезной нагрузке действия карты. Каждый навык должен встраить `skillId` в  `value` область действий карт. Вы должны убедиться, что каждое действие карты несет соответствующие сведения о навыках, и Виртуальный помощник использовать эту информацию для отправки.

Необходимо предоставить в `skillId` конструкторе сведения о навыках, которые всегда присутствуют в действиях карт.
...
Далее класс `SkillCardActionData` в шаблоне Виртуальный помощник для извлечения `skillId` из полезной нагрузки действия карты.
Фрагмент кода, извлеченный из  `skillId` полезной нагрузки действия карты, показан в следующем разделе:
...
Фрагмент кода, извлеченный из  `skillId` данных действий карты, показан в следующем разделе:
...
Чтобы добавить возможности модуля задач в Виртуальный помощник, в обработнике Виртуальный помощник действий включаются два дополнительных метода: `OnTeamsTaskModuleFetchAsync` и `OnTeamsTaskModuleSubmitAsync`. Эти методы прослушивают действия, связанные с модулем задач, Виртуальный помощник, определяют навыки, связанные с запросом, и переназначяют запрос в указанный навык.

Переададка запросов делается [методом SkillHttpClient](/dotnet/api/microsoft.bot.builder.integration.aspnet.core.skills.skillhttpclient?view=botbuilder-dotnet-stable&preserve-view=true)`PostActivityAsync`. Он возвращает ответ, как `InvokeResponse` который разобрано и преобразовано в `TaskModuleResponse` .
...
Аналогичный подход следует для отправки действий по картам и ответов модулей задач. Модуль задач для получения и отправки данных действий обновляется, чтобы включить `skillId`.
Метод расширения действий `GetSkillId` извлекает `skillId` из полезной нагрузки, которая содержит сведения о навыке, который необходимо вызвать.
...
Кроме того, `validDomains` необходимо включить все домены навыков в разделе Виртуальный помощник манифеста, чтобы модули задач, вызываемые с помощью отрисовки навыков должным образом.

### <a name="handle-collaborative-app-scopes"></a>Обработка областей совместных приложений

Teams приложения могут существовать в нескольких сферах, включая чат 1:1, групповой чат и каналы. Основной шаблон Виртуальный помощник предназначен для чатов 1:1. В рамках пользовательского интерфейса Виртуальный помощник пользователям имя и поддерживает состояние пользователя. Так как он не подходит для групповых чатов или областей каналов, он удален.
...
После вызова команд пользователем Виртуальный помощник определить соответствующий навык путем разбора ИД команды, `:<skill_id>` обновления действия путем удаления дополнительного суффикса из командного ИД и его переад. Код для навыка не должен обрабатывать дополнительный суффикс. Таким образом, можно избежать конфликтов между командными ИД между навыками. С помощью этого подхода все команды поиска и действий навыка во всех контекстах, таких как **compose**, **commandBox** и сообщение, будут  работать на Виртуальный помощник.
...
Некоторые действия по расширению обмена сообщениями не включают командный ID. Например, содержит `composeExtension/selectItem` только значение действия касания. Чтобы определить связанный навык, `skillId`  при формировании ответа для каждого элемента присоединяется к каждой карте элемента `OnTeamsMessagingExtensionQueryAsync`. Это похоже на подход к [добавлению адаптивных карт](#add-adaptive-cards-to-your-virtual-assistant) в Виртуальный помощник.
...
В следующем примере показано, как преобразовать шаблон приложения Book-a-room в навык Виртуальный помощник: Book-a-room — это Microsoft Teams, который позволяет пользователям быстро находить и резервировать зал собраний на 30, 60 или 90 минут начиная с текущего времени. Время по умолчанию — 30 минут. Область бота Book-a-room для личных или 1:1 бесед.
На следующем изображении отображается Виртуальный помощник с **книгой навыков** комнаты:
...
Виртуальный помощник в формате в формате для ввода при присоединении навыка требуется модель LUIS`.lu`. LUIS json преобразуется в `.lu` формат с помощью средства botframework-cli.
...
Мы создали модель LUIS, понимая эти две команды. Соответствующие секреты должны быть заполнены в `cognitivemodels.json`. Соответствующий файл LUIS JSON находится [здесь](https://github.com/OfficeDev/microsoft-teams-apps-bookaroom/blob/nebhagat/microsoft-teams-apps-bookaroom-skill/Deployment/Resources/LU/en-us/book-a-meeting.json).
...
С помощью этого `book room` `manage favorites` `Book-a-room` подхода любая команда, выданная пользователем для Виртуальный помощник или идентифицирована как команда, связанная с ботом, и передается на этот навык.
С другой стороны, боту `Book-a-room room` необходимо использовать модель LUIS, чтобы понять эти команды, если они не введите полный текст. Пример: `I want to manage my favorite rooms`.
...
Параллельно добавьте соответствующий файл `.lu` в путь LuisFolder. Структура папок должна быть следующим образом:
...
Дополнительные сведения о добавлении проверки утверждений в навык см. в добавлении проверки [утверждений к навыку](/azure/bot-service/skill-implement-skill?view=azure-bot-service-4.0&tabs=cs#claims-validator&preserve-view=true).

### <a name="limitation-of-card-refresh"></a>Ограничение обновления карты

Обновление активности, например обновления карт, еще не поддерживается Виртуальный помощник ([проблема github](https://github.com/microsoft/botbuilder-dotnet/issues/3686)). Таким образом, мы заменили все вызовы обновления карт `UpdateActivityAsync` на размещение новых вызовов карт `SendActivityAsync`.
...
`Book-a-room bot` предназначена только для частных чатов, например личных или 1:1. Так как мы Виртуальный помощник для поддержки групповых чатов и областей каналов, Виртуальный помощник должны вызываться из областей канала, `Book-a-room` поэтому бот должен получать действия для той же области. Поэтому `Book-a-room`бот настраивается для обработки этих действий. Вы можете найти способы проверки `OnMessageActivityAsync` методов `Book-a-room` обработки действий бота.
...
Вы также можете использовать существующие навыки из [репозитория Bot Framework Solutions](https://github.com/microsoft/botframework-components/tree/main/skills/csharp) или создать новый навык с нуля. Для создания нового навыка см. [в руководстве по созданию нового навыка](https://microsoft.github.io/botframework-solutions/overview/skills/). В Виртуальный помощник документации по архитектуре и навыкам см. Виртуальный помощник [и архитектуру навыков](/azure/bot-service/skills-conceptual?view=azure-bot-service-4.0&preserve-view=true).  

## <a name="limitations-of-virtual-assistant"></a>Ограничения Виртуальный помощник

* **EndOfConversation**. Навык должен отправлять `endOfConversation` действие по завершении беседы. На основе этого действия Виртуальный помощник контекст с определенным навыком и возвращается Виртуальный помощник корневого контекста. Для бота Book-a-room нет четкого состояния, в котором беседа завершена. Поэтому мы не отослали `endOfConversation` `Book-a-room` от бота, и когда пользователь хочет вернуться к корневому контексту, он может просто сделать это по команде `start over` .  
