...
В этом руководстве вы можете создать базовое бот-приложение. Бот выступает в качестве посредника между Teams пользователями и вашим веб-приложением или службой с диалоговом интерфейсом. Люди могут общаться с ботом, чтобы быстро получить информацию или инициировать рабочий процесс и задачи, выполняемые вашей службой.

## <a name="what-youll-learn"></a>Что вы узнаете

* Создайте проект приложения и бот с помощью Microsoft Teams набор средств для Visual Studio Code.
* Понимание конфигураций Teams приложений, соответствующих ботам.
...
В Microsoft Teams набор средств вы можете настроить следующие компоненты для приложения: 

* **Конфигурации приложений и леса, соответствующие** ботам.
* **Бот,** автоматически зарегистрированный в службе Microsoft Azure бота.
...
1. Выберите **Создание нового bot** Create Bot  >  **Registration,** как показано на следующем изображении:
    ...
    В случае успешной работы новый бот будет иметь **зарегистрированный** статус. Теперь бот автоматически регистрируется в службе Microsoft Azure бота. 
...
Большая часть конфигураций приложений и строительных лесов создается автоматически при создании проекта с помощью Teams набор средств. Рассмотрим основные компоненты для создания бота:
...
Если вы создали вкладку в другом учебнике, строительные леса приложений для бота отличаются. В отличие от вкладок, для разработки бота не требуется создавать какие-либо веб-компоненты переднего Teams клиента JavaScript.  Вместо этого в строительных лесах используется [Microsoft Bot Framework](https://dev.botframework.com/)SDK с открытым исходным кодом для создания интеллектуальных корпоративных ботов, которые могут работать в Интернете, мобильных устройствах и, конечно, Teams! 

Файл, расположенный в корневом каталоге проекта, является обработщиком Teams, который обрабатывает действия бота, например, как бот реагирует на определенные `botActivityHandler.js` сообщения. Строительные леса приложений предоставляют файл, расположенный в корневом каталоге проекта, это специальный обработщик Teams который обрабатывает действия бота, например, как бот реагирует на определенные `botActivityHandler.js` сообщения.
...
1.  После завершения настройки откройте терминал и запустите `ngrok http -host-header=rewrite 3978` . 
    ...
    Теперь ngrok предоставляет общедоступный защищенный URL-адрес, который передается в локальный адрес в порту 3978, поэтому скопируйте URL-адрес HTTPS, например, как показано на скриншоте ниже, так как для Teams требуется `https://287a4f4223bc.ngrok.io` подключение HTTPS: 
...
Для получения и обработки сообщений пользователей или запросов, отправленных боту, необходимо указать адрес конечной точки. Как правило, URL-адрес выглядит `https://HOST_URL/api/messages` как . Это можно настроить в наборе инструментов.

1. В Visual Studio Code откройте **Microsoft Teams набор средств**.
...
1. В поле **адресов конечной точки** Бота введите URL-адрес ngrok, например, где размещен бот и `https://287a4f4223bc.ngrok.io` приложение к `/api/messages` ней:
    ...
    Ваш бот сможет отвечать на сообщения в Teams после правильного настройка конечной точки. 
...
Если вы установили приложение, но бот не работает, убедитесь, что бот подключен к каналу службы Teams Azure [Bot. ](/azure/bot-service/channel-connect-teams?view=azure-bot-service-4.0&preserve-view=true)
...
* [Узнайте, что Teams боты могут сделать с одним из наших образцов](https://github.com/microsoft/BotBuilder-Samples#teams-samples)
