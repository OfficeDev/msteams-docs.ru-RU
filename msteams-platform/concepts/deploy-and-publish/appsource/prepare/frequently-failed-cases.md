...
* Не внося изменения в приложение во время процесса проверки. Для этого потребуется полная переоценка приложения.
...
* [Проверка подлинности с вкладками и ботами в C#/.NET](https://github.com/OfficeDev/microsoft-teams-sample-complete-csharp)
...
* На экране конфигурации должно быть четко по-разъяснено значение работы и как настроить вкладку.
...
* Вход пользователя должен быть частью процесса настройки и должен завершиться в пользовательском интерфейсе tab. После завершения настройки и загрузки вкладки никаких дополнительных действий не требуется.
...
* Интерфейс настройки должен предоставить пользователю возможность найти контент, закрепить URL-адрес или создать новый контент, если он не существует.
...
Приветствие — лучший способ настроить тон для вашего бота личного чата. Это первое взаимодействие пользователя с ботом. Хорошее приветствие может побудить пользователя продолжать изучать приложение. Если приветствие или вводное сообщение является запутанным или нечетким, пользователи не сразу увидят значение приложения и потеряют интерес.
...
* Никогда не отправлять приветствие более одного раза. Повтор одного и того же приветствия через регулярные интервалы не разрешен и считается нежелательным.
...
* **Личный чат от бота.** Не отправляйте несколько сообщений. Отправьте одно сообщение с полной информацией. Повтор одного и того же приветствия через регулярные интервалы не разрешен и считается нежелательным.
...
* Связанные страницы политики конфиденциальности и условий использования также не должны иметь коммерческих ссылок на пользовательский интерфейс или магазин.
...
* Ваше приложение Teams не должно просить пользователей внести платеж непосредственно в интерфейсе Teams
