...
ms.locfileid: "60566311"
---
# <a name="tips-for-a-successful-microsoft-teams-app-submission"></a>Советы для успешной отправки Microsoft Teams приложения

>[!NOTE]
>Эта страница будет обесценяться до мая 2021 г. Дополнительные сведения об успешной публикации приложения см. в Teams руководства по проверке [хранения.](~/concepts/deploy-and-publish/appsource/prepare/teams-store-validation-guidelines.md)

В этой статье представлены распространенные причины неудачной проверки отправленных приложений. Хотя это не исчерпывающий список всех потенциальных проблем с приложением, следуя этому руководству, вероятность того, что отправка приложения пройдет в первый раз, увеличится. Дополнительные сведения см. в примере [Политики](/legal/marketplace/certification-policies) сертификации коммерческих рынков для обширного списка политик проверки.

>[!NOTE]
>**[Раздел 1140](/legal/marketplace/certification-policies#1140-teams)** посвящен Microsoft Teams и в разделе **[1140.4](/legal/marketplace/certification-policies#11404-functionality)** адресов требования к функциональным возможностям Teams приложений.
...
* Визуальный тур может быть видео, скриншот, ссылка на статическую вкладку с сведениями о приложении. Все эти ссылки должны быть в Teams среде.
...
* Приложение не должно принимать пользователей из-за Teams для основных сценариев пользователей. Целевые объекты ссылок в приложениях не должны связываться с внешним браузером. Целевые объекты ссылок должны ссылаться на элементы div, содержащиеся в Teams, например, модули задач и вкладки. 
...
* Параметр выхода должен только подписывать пользователя из возможностей вашего приложения, а не из Teams клиента.
...
* Убедитесь, что всегда существует способ обратить вспять следующее (или аналогичное) поведение:
  ...
  * Подключение учетную запись/службу => отключить учетную запись/службу.
...
* Приложения без простого потока регистрации также должны включать вкладку справки или ссылку на веб-страницу, где новый пользователь может видеть подробные указания по настройке Teams приложения. Предостереите подробные сведения, чтобы новый пользователь не был заблокирован при первой попытке приложения.
* Функции входов и регистрации должны работать на мобильных клиентах. Убедитесь, [что Microsoft Teams версии SDK](https://www.npmjs.com/package/@microsoft/teams-js) 1.4.1 или более поздней версии.
...
* Вкладки канала не должны предоставлять панели приложений с значками на левом рельсе, что противоречит основному Teams навигации.
* Вкладки не должны представлять панели приложений с значками на левом рельсе, которые конфликтуют с основным Teams навигацией.
...
* Опыт конфигурации должен оставаться в Teams контексте. Пользователю не нужно оставлять интерфейс конфигурации для создания контента, а затем возвращаться к Teams, чтобы закрепить его.
...
* **Используйте адаптивные карты и модули задач,** чтобы сделать ответ бота понятным и действий Адаптивные карточки с кнопками вызова модулей задач улучшают пользовательский интерфейс бота. Эти карточки и кнопки проще использовать на мобильном устройстве в отличие от ввода команд пользователем. Кроме того, ответы бота не должны быть текстовыми с длинным текстом. Боты должны использовать адаптивные карты и модули задач вместо пользовательского интерфейса на основе беседы и длительных текстовых откликов.

* **Продумайте все области.** Убедитесь, что ваш бот предоставляет соответствующие ответы при упоминаний () в `@*botname*` канале и в личных беседах. Если ваш бот не обеспечивает значимый контекст в области личных или команд, отключите эту область с помощью манифеста. (См. `bots` блок в ссылке Microsoft Teams [манифеста](../../../../resources/schema/manifest-schema.md#bots).)
...
* Представляем простой для чтения текст и прямой диалог — желательно карточку с кнопкой приветствия, загружаемой модулем задач.
...
* **Личный чат по боту**. Не отправлять несколько сообщений. Вместо этого отправьте одно сообщение со всей информацией. Повторение одного и того же приветствия через регулярные интервалы не допускается и считается нежелательной почтой.
...
* Ваши вкладки, адаптивные карты, сообщения ботов и содержимое в модулях задач должны отвечать на различные размеры экранов мобильных устройств.
* Приложения, поддерживают iOS, должны быть полностью функциональными iPad устройстве с использованием последней версии iOS.
...
Приложение Teams не должно размещать конфиденциальные данные, такие как кредитная карточка или инструмент финансовых платежей, личная идентифицируемая информация (PIN-код), состояние здоровья или контактные данные отслеживания для аудитории, не предназначенной для просмотра этих данных.

### <a name="9989-do-not-transmit-financial-payment-details-or-complete-financial-transactions-via-your-teams-app"></a>&#9989; Не передайте сведения о финансовых платежах и не завершайте финансовые операции с помощью Teams приложения

* Ваше Teams не должно просить пользователей сделать оплату непосредственно в Teams интерфейсе.
...
## <a name="m365-publisher-attestation"></a>M365 Publisher проверки

### <a name="9989-complete-the-publisher-attestation-in-partner-center"></a>&#9989; завершить проверку Publisher в Центре партнеров

* Дополнительные сведения [можно найти в документации по программе Publisher](/microsoft-365-app-certification/docs/attestation) полной проверки.
* Выполните действия в [разделе рабочий процесс Publisher](/microsoft-365-app-certification/docs/userguide#3publisher-attestation-workflow) проверки, чтобы завершить процесс проверки издателя. Напишите appcert@microsoft.com для любых вопросов.
...
* [Дополнительные дополнительные Teams политик утверждения приложений](/legal/marketplace/certification-policies#1140-teams)
* [Раздел 100 — общие](/legal/marketplace/certification-policies#100-general)
* [Раздел 1100.5 — управление клиентами](/legal/marketplace/certification-policies#11005-customer-control)
