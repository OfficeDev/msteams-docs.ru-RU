...
Имена функций приложения в кнопках и другом тексте пользовательского интерфейса не должны противоречить терминологии, зарезервированной для Teams и других продуктов Майкрософт (например, Start **meeting,** **Make call**, or **Start chat).** Включите имя приложения, если этого не удалось полностью избежать, например собрание **Start Contoso** вместо **start meeting.**
...
* Глубокие ссылки на вкладки должны быть связаны не с внешней веб-страницей, а с Teams (например, модулями задач или другими вкладками).
...
* Вкладки должны использовать Teams(Teams шрифты, пандусы типа, цветовую палитру, систему сетки, движение, тон голоса и т. д.).
* Необходимо включить **вкладку Параметры.**
* Вкладки должны следовать Teams взаимодействия (навигация на странице, расположение и использование диалогов, информационных иерархий и т.д.) по мере возможности.
* Содержимое вкладки в iframe не должно включать функции, имитирующие Teams основные возможности (например, боты, расширения обмена сообщениями, вызовы, собрания и т.д.).
...
> [!TIP]
> В приветствиях отдельным пользователям тур карусель может предоставить эффективный обзор бота и любых других функций приложения. Рекомендуется включить кнопки, с помощью которых пользователи смогут попробовать команды бота (например, **Создать задание**).
...
* Вкладки должны иметь организованный макет при отобратии нескольких элементов (например, более 10 опросов или опросов). См. [пример макета](~/apps-in-teams-meetings/design/designing-apps-in-meetings.md#after-a-meeting).
...
Если ваше приложение использует API-API каналов активности, предоставляемые [корпорацией Майкрософт Graph, убедитесь,](https://docs.microsoft.com/graph/teams-send-activityfeednotifications)что оно придерживается следующих рекомендаций.
