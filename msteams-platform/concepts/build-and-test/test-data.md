...
description: Настройка подписки Office 365 разработчика для успешного тестирования Microsoft Teams приложений с помощью фрагментов кода
ms.topic: how-to
...
ms.locfileid: "63452706"
---
# <a name="add-test-data-to-your-microsoft-365-test-tenant"></a>Добавление тестовых данных в Microsoft 365 тестовый клиент

Вы можете проверить Microsoft Teams с примерными данными с помощью подписки Microsoft 365 разработчика.
...
1. Сохраните следующий фрагмент в **качестве.xmlи** обратите внимание на путь файла. Этот XML определяет структуру команды и канала, созданного вместе с ее участниками:
...
4. Если вам будет предложено предоставить учетные данные, введите учетные данные глобального администратора, полученные при первой подписке на разработчика.
    ...
    > [!Note]
    > Не закрывайте сеанс PowerShell, так как для выполнения сценария требуется несколько минут. Если вы изменили пользователей подписки из того, что создано в пакете контента по умолчанию, некоторые пользователи не могут быть добавлены Teams. При выполнении скрипта отображаются успешные или неудачные действия.
