...
* `attendees`: необязательный список идентификаторов пользователей, разделенный запятыми, представляющий участников собрания. Пользователь, выполняющий действие, является организатором собрания. В настоящее время поле ID пользователя поддерживает только UserPrincipalName из Microsoft Azure Active Directory (Azure AD) - как правило, это адрес электронной почты.
...
> [!NOTE]
> В настоящее время прямая ссылка на вызов не поддерживается на мобильных устройствах Teams.
...
* `users`: разделенный запятыми список идентификаторов пользователей, представляющих участников звонка. В настоящее время поле User ID поддерживает UserPrincipalName из Microsoft Azure Active Directory, обычно адрес электронной почты или, в случае звонка по ТСОП, значение pstn mri 4:&lt;phonenumber&gt;.
