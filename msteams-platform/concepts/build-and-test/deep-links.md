...
Чтобы реализовать это, необходимо добавить действие **копирования ссылки** к каждому элементу любым способом, который лучше всего подходит для пользовательского интерфейса. Когда пользователь выполняет это действие, вызовите [pages.shareDeepLink()](/javascript/api/@microsoft/teams-js/pages?view=msteams-client-js-latest#@microsoft-teams-js-pages-sharedeeplink&preserve-view=true) для отображения диалогового окна, содержащего ссылку, которую пользователь может скопировать в буфер обмена. При выполнении этого вызова передайте идентификатор для вашего элемента, который возвращается в [контексте](~/tabs/how-to/access-teams-context.md), при переходе по ссылке и перезагрузке вкладки.
...
Хотя рекомендуется использовать строго типизированные API, вы также можете использовать следующий формат для созданной вручную прямой ссылки, которую можно использовать в боте, соединителе или карточке расширения сообщения:
...
Хотя рекомендуется использовать строго типизированные API-интерфейсы TeamsJS, вы также можете использовать созданную вручную прямую ссылку, чтобы начать вызов.
