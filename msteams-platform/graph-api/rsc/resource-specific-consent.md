...
Если администратор Teams позволяет настраивать загрузки приложений, вы можете загрузить приложение непосредственно в [](~/concepts/deploy-and-publish/apps-upload.md) определенную группу или чат.
...
1. Вопишитесь **в Graph Explorer**.
1. Сделайте **вызов GET** в эту конечную точку: `https://graph.microsoft.com/beta/teams/{teamGroupId}/permissionGrants`. Поле `clientAppId` в ответе будет соедино `webApplicationInfo.id` указанному в манифесте Teams приложения.
...
1. Вопишитесь **в Graph Explorer**.
1. Вызов **GET на** следующую конечную точку: `https://graph.microsoft.com/beta/chats/{chatId}/permissionGrants`. Поле `clientAppId` в ответе будет соедино `webApplicationInfo.id` указанному в манифесте Teams приложения.
