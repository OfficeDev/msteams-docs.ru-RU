...
- **TeamsFx CLI**: ускоряет Teams разработку приложений. Он также включает сценарий CI/CD, в котором можно интегрировать CLI в скрипты для автоматизации.

- **TeamsFx SDK**: TeamsFx Software Development Kit (SDK) является основной библиотекой кодов TeamsFx, инкапсулируя простую проверку подлинности для клиентского и серверного кода, адаптированного для Teams разработчиков.
...
> [!NOTE]
> Журналы фоновых служб, таких как React, сохраняются в `~/.fx/cli-log/local-preview/`.
...
Project разработчики и сотрудники могут использовать команду `teamsfx permission grant` для добавления нового соавтора в проект:
