...
### <a name="azure-ad-parameters"></a>параметры Azure AD
...
| Ресурс | Назначение | Описание |
| --- | --- | --- |
| План службы приложений | Размещает веб-приложение вкладки. | Неприменимо |
...
| Управление удостоверениями | Проверка подлинности запросов между службами Azure. | Совместно использует различные возможности и ресурсы. |
...
| Ресурс | Назначение | Описание |
| --- | --- | --- |
| Бот Azure | Регистрирует приложение в качестве бота с помощью платформы bot. | Подключает бота к Teams. |
| план Служба приложений | Размещает приложение веб-бота. | Неприменимо |
...
<details>
<summary><b>Создание ресурса для бота уведомлений Teams с приложением триггера HTTP (сервер веб-API)</b></summary>

| Ресурс | Назначение | Описание |
| --- | --- | --- |
| Бот Azure | Регистрирует приложение в качестве бота с помощью платформы bot. | Подключает бота к Teams. |
| План службы приложений | Размещает приложение веб-бота. | Неприменимо |
| Служба приложений | Разместите приложение бота. | Добавляет назначенное пользователем удостоверение для доступа к другим ресурсам Azure. |
| Управляемое удостоверение | Проверка подлинности запросов между службами Azure. | Совместно использует различные возможности и ресурсы. |

</details>
<br>

<details>
<summary><b>Создание ресурса для бота уведомлений Teams с приложением триггера HTTP (функция Azure)</b></summary>

| Ресурс | Назначение | Описание |
| --- | --- | --- |
| Бот Azure | Регистрирует приложение в качестве бота с помощью платформы bot. | Подключает бота к Teams. |
| Управление удостоверениями | Выполняет проверку подлинности запросов между службами Azure. | Совместное управление разными возможностями и ресурсами. |
| Учетная запись хранения | Помогает создать приложение-функцию. | Неприменимо |
| План службы приложений | Размещает приложение-бот-функцию. | Неприменимо |
| Приложение-функция | Размещает приложение бота. | — Добавляет назначаемое пользователем удостоверение для доступа к другим ресурсам Azure.<br>—Добавляет правило общего доступа к ресурсам между источниками (CORS), разрешая запросы из приложения вкладки.<br>— Добавляет параметр проверки подлинности, разрешающий только запросы из приложения Teams.<br>— Добавляет параметры приложения, необходимые для TeamsFx SDK. |

</details>
<br>

<details>
<summary><b>Создание ресурса для бота уведомлений Teams с приложением триггера таймера (функция Azure)</b></summary>
...
<details>
<summary><b>Создание ресурса для бота уведомлений Teams с помощью приложения триггера HTTP и триггера таймера (функция Azure)</b></summary>
...
Файлы шаблонов ARM используют заполнители для параметров. Цель заполнителей — обеспечить возможность создания новых ресурсов в новой среде. Фактические значения разрешаются из `.fx/states/state.{env}.json` файла.
