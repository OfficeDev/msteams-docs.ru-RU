...
> [!NOTE]
> В настоящее время неинактивная проверка подлинности для M365 используется в рабочего процессах CI или CD, чтобы учетная запись M365 была достаточной привилегией в клиенте и не была включена многофакторная проверка подлинности или другие расширенные функции безопасности. Дополнительные сведения см. в перенастройке учетных данных [M365,](https://github.com/OfficeDev/teamsfx-cli-action/blob/main/README.md#configure-m365azure-credentials-as-github-secret) чтобы убедиться, что вы отключили многофакторную проверку подлинности и по умолчанию для учетных данных, используемых в рабочего процесса.
