---
title: Управление приложением Azure Active Directory в Наборе средств Teams
author: zyxiaoyuer
...
---
# <a name="customize-azure-ad-manifest"></a>Настройка манифеста Azure AD
...
Набор средств Teams теперь Azure AD приложения с файлом манифеста в качестве источника достоверных сведений в течение жизненного цикла разработки приложений Teams.
...
Она отличается от официальной схемы манифеста [Azure AD](/azure/active-directory/develop/reference-app-manifest) `resourceAppId` `resourceAccess` `requiredResourceAccess`, которая и идентификатор в свойстве поддерживают только UUID, шаблон манифеста Azure AD в Наборе средств Teams `Microsoft Graph` `Office 365 SharePoint Online` также поддерживает удобочитаемые пользователем строки и разрешения. При вводе UUID в code lens отображаются удобочитаемые строки пользователя, в противном случае — UUID.
...
1. **Создание проекта**
      ...
      Вы можете создать проект с набором средств Teams, который поставляется с поддержкой единого входа по умолчанию `SSO-enabled tab`, например . Дополнительные сведения о создании нового приложения см. в статье ["Создание приложения Teams с помощью Набора средств Teams"](create-new-project.md). Автоматически создается Azure AD манифеста. `templates\appPackage\aad.template.json` Teams Toolkit создает или обновляет Azure AD во время локальной разработки или при перемещении приложения в облако.
...
3. **Локальное построение**
    ...
    Набор средств Teams выполняет следующие функции во время локальной разработки (известной как F5):

    * Прочитайте файл`state.local.json`, чтобы найти существующее Azure AD приложения. Если приложение Azure AD уже существует, Teams Toolkit повторно использует существующее приложение Azure AD в противном случае необходимо создать новое приложение с помощью `aad.template.json` файла.
    ...
    * Изменения, внесенные в Azure AD приложения, загружаются во время следующего локального сеанса отладки. Вы можете [просмотреть Azure AD приложения,](https://github.com/OfficeDev/TeamsFx/wiki/) чтобы применить изменения вручную Azure AD изменения приложения.
...
4. **Подготовка облачных ресурсов**
      ...
      * Прочитайте файл`state.{env}.json`, чтобы найти существующее Azure AD приложения. Если приложение Azure AD уже существует, Teams Toolkit повторно использует существующее приложение Azure AD в противном случае необходимо создать новое приложение с помощью `aad.template.json` файла.

      * Изначально игнорирует некоторые свойства в файле манифеста, для которых требуется дополнительный контекст (например, свойство replyUrls требует интерфейсную или конечную точку бота) во время создания нового приложения Azure AD с файлом манифеста.
