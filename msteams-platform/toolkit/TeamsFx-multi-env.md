...
1. **Тест перед** производством. Это распространенная практика, чтобы настроить несколько сред, таких как разработчик, тест и постановка перед публикацией приложения Teams для производственной среды в современном жизненном цикле разработки приложений.
...
> [!TIP]
> Убедитесь, Teams проект приложения открыт в Microsoft Visual Studio коде.
...
Вы можете Teams имя `myapp(dev)` `dev` `myapp(staging)` приложения для среды по умолчанию и среды постановок.`staging`
...
- 1. Откройте файл config `.fx/configs/config.dev.json`.
- 2. Добавьте новое свойство *манифеста > описание > со* значением `my app description for dev`.
