...
---
# <a name="create-a-bot-for-teams"></a>Создать бота для Teams
...
> * Определите свойства манифеста приложения и некоторые шаблоны формирования шаблонов, относящиеся к Боты
> * Размещение ленты на локальном компьютере
> * Настройка почтового робота для Teams
> * Загрузка неопубликованных и тестирование ленты в Teams

## <a name="before-you-begin"></a>Прежде чем начать

Если вы еще не сделали это, настройте [учетную запись](building-real-world-app.md#set-up-your-development-account) Microsoft 365 Development и [средства Teams](building-real-world-app.md#install-your-development-tools).

## <a name="create-your-app-project-and-bot"></a>Создание проекта приложения и ленты

Набор средств Microsoft Teams помогает настроить следующие компоненты для вашего приложения:

* **Проект приложения**: включает манифест приложения и формирование шаблонов, относящихся к Боты
* **Bot**: настраивает новый Bot и регистрирует его в службе Microsoft Azure Bot
...
* `title`: Имя команды Bot пользователи видят в клиенте Teams.
...
1. В поле **адрес конечной точки Bot** введите локальный веб-сервер, на котором размещается Bot, и добавьте `/api/messages` к нему.
...
1. Перейдите к нужному элементу Bot и выберите только что неопубликованные.<br/>
...
> [!NOTE]
> Если вы вносите изменения в код после проверки ленты (например, обновление `botActivityHandler.js` ), необходимо повторно запустить приложение, чтобы увидеть изменения, отраженные в Teams.
...
* [Узнайте, какие другие возможности Teams Боты могут делать с одним из наших примеров (GitHub).](https://github.com/microsoft/BotBuilder-Samples#teams-samples)
