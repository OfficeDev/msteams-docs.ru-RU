...
Расширения обмена сообщениями на основе действий позволяют пользователям запускать действия во внешних службах во время работы в Teams.
...
Вы можете протестировать расширение обмена сообщениями, загрузив приложение. Подробные [сведения см. в материале Uploading your app in a team.](~/concepts/deploy-and-publish/apps-upload.md)
...
Существует три способа сбора сведений от конечных пользователей в Teams.
...
Когда пользователь выбирает команду со статичными параметрами, Teams создает форму в модуле задач с параметрами, определенными в манифесте. При нажатии Отправить `composeExtension/submitAction` отправляется боту. Дополнительные [сведения](#responding-to-submit) о ожидаемом наборе ответов см. в разделе Ответы.
...
В этом методе ваша служба получит событие и должна ответить адаптивной картой на основе ответа модуля `composeExtension/fetchTask` [задач](~/task-modules-and-cards/what-are-task-modules.md#the-taskinfo-object). Ниже приведен пример ответа с адаптивной картой:
...
Так же, как и в потоке адаптивной карты, служба будет отправлять событие и должна отвечать ответом модуля задач на основе `fetchTask` [URL-адреса.](~/task-modules-and-cards/what-are-task-modules.md#the-taskinfo-object) Ниже приведен пример ответа с адаптивной картой:
...
Чтобы облегчить этот поток, при первом выполнении расширения обмена сообщениями выполняется проверка вызова, чтобы узнать, установлен ли ваш бот в текущем контексте (вы можете выполнить это, например, при попытке вызова реестра `composeExtension/fetchTask` получения). Если бот не установлен, возвращается адаптивная карта с действием, которое запрашивает у пользователя установку бота См. в примере ниже. Обратите внимание, что для этого пользователю необходимо иметь разрешение на установку приложений в этом расположении; если они не могут, им будет представлено сообщение с просьбой связаться со своим администратором.
...
Это используется, когда для продолжения расширения необходимо либо проверить подлинность, либо настроить. Дополнительные [сведения см.](~/resources/messaging-extension-v3/search-extensions.md#authentication) в разделе проверка подлинности в разделе поиск.
...
В этом примере показан этот поток с помощью [SDK Microsoft.Bot.Connector.Teams (v3).](https://www.nuget.org/packages/Microsoft.Bot.Connector.Teams)
