...
description: Из этой статьи вы узнаете, как разрабатывать расширения сообщений на основе поиска.
ms.topic: how-to
...
Если вы задано `canUpdateConfiguration` значение `true` в манифесте, вы включаете пункт меню "Параметры" для расширения сообщения, а также должны обрабатывать `onQuerySettingsUrl` и `onSettingsUpdate`.
...
События `onQuerySettingsUrl` и `onSettingsUpdate` события работают вместе, чтобы включить пункт меню **"Параметры** ".

![Снимки экрана: расположения пункта меню "Параметры"](~/assets/images/compose-extensions/compose-extension-settings-menu-item.png)
...
Когда пользователь выполняет запрос, Microsoft Teams отправляет службе стандартный объект Bot Framework `Activity` . Служба должна выполнять логику для `Activity` `name` `type` `invoke` `composeExtension` поддерживаемого типа, как показано в следующей таблице.
...
В качестве альтернативы (или в дополнение) к поиску во внешней службе можно использовать URL-адрес, вставленный в поле сообщения создания, для запроса службы и возврата карточки. На снимке экрана ниже пользователь вставляет URL-адрес рабочего элемента в Azure DevOps, который расширение сообщения разрешается в карточку.

![Пример развертывания ссылки](~/assets/images/compose-extensions/messagingextensions_linkunfurling.png)

Чтобы расширение сообщений взаимодействовать со ссылками таким образом, сначала необходимо добавить массив в манифест приложения, `messageHandlers` как показано в примере ниже:
...
Когда пользователь выполняет запрос, Teams выполняет синхронный HTTP-запрос к службе. В течение этого времени код предоставляет http-ответ на запрос в течение 5 секунд. В течение этого времени служба может выполнить дополнительный поиск или любую другую бизнес-логику, необходимую для обслуживания запроса.
...
Вы можете отобразить предварительную версию адаптивной или Office 365 соединителя в списке результатов, просто настроив ее свойство предварительного просмотра. Это необязательно, если результаты уже являются карточками имиджевого баннера или эскиза. Если вы используете вложение предварительного просмотра, оно должно быть карточкой hero или Thumbnail. Если свойство предварительного просмотра не указано, предварительный просмотр карточки завершится ошибкой и ничего не будет отображаться.
...
Если вы заданы `initialRun` `true` в манифесте, Microsoft Teams выполняет запрос по умолчанию, когда пользователь впервые открывает расширение сообщения. Ваша служба может отвечать на этот запрос с помощью набора предварительно заполненных результатов. Это может быть полезно для отображения, например, недавно просматриваемых элементов, избранного или любой другой информации, которая не зависит от введенных пользователем данных.
...
Каждый запрос к службам включает скрытый идентификатор пользователя, выполнив запрос, а также отображаемое имя пользователя и Microsoft Azure Active Directory (Azure AD) объекта.
...
5. После входа пользователя следует закрыть окно и отправить "код проверки подлинности" клиенту Teams.
6. Затем клиент Teams повторно передает запрос в службу, которая включает код проверки подлинности, переданный на шаге 5.

Ваша служба должна убедиться, что код проверки подлинности, полученный на шаге 6, соответствует коду из шага 5, что гарантирует, что злоумышленник не попытается подделывать или скомпрометировать поток входа. Это фактически "закрывает цикл" для завершения последовательности надежной проверки подлинности.
...
> [!NOTE]
> Чтобы интерфейс входа размещался во всплывающем окне Teams, доменная часть URL-адреса должна находиться в списке допустимых доменов приложения. Дополнительные сведения см. в разделе [validDomains](~/resources/schema/manifest-schema.md#validdomains) в схеме манифеста.
...
Как и в случае с другими встроенными интерфейсами, выполняющиеся в Teams, код в окне должен сначала вызвать.`microsoftTeams.initialize()` Если код выполняет поток OAuth, вы можете передать идентификатор пользователя Teams в окно, который затем можно передать по URL-адресу для входа в OAuth.
...
На этом этапе окно закрывается, и управление передается клиенту Teams. Теперь клиент может повторно выполнить исходный пользовательский запрос вместе с кодом безопасности в свойстве `state` . Ваша программа может использовать код безопасности для поиска учетных данных, сохраненных ранее, для завершения последовательности проверки подлинности, а затем для выполнения запроса пользователя.
