...
Как и в случае с другими встроенными Teams, код в окне должен сначала вызвать.`microsoftTeams.initialize()` Если код выполняет поток OAuth, вы можете передать идентификатор пользователя Teams в окно, который затем можно передать по URL-адресу для входа в OAuth.
