...
keywords: файл cookie атрибуты samesite; cookie attributes samesite
ms.topic: reference
...
|Cookie|Область|
| ------ | ------ |
|Основной файл cookie|Основной файл cookie создается веб-сайтами, которые посещает пользователь. Он используется для сохранения данных, таких как сведения о товарах в корзине, учетные данные для входа. Например, файлы cookie проверки подлинности и другая аналитика.|
...
|Сторонний файл cookie|Сторонний файл cookie устанавливается доменом, отличным от домена, который пользователь явно посетил, и в основном используется для отслеживания. Например, кнопки **Нравится**, доставка рекламы и чаты в реальном времени.|
...
| **Нет** | Файлы cookie отправляются как в контексте первой стороны, так и в межсайтовых запросах. При этом должно быть явно задано значение **`None`** и все запросы браузера **должны выполняться в соответствии с протоколом HTTPS** и включать атрибут **`Secure`**, для которого требуется зашифрованное подключение. Файлы cookie, которые не соответствуют данному требованию, **отклоняются**. <br/>**Наличие обоих атрибутов обязательно.** Если **`None`** указывается без **`Secure`** или если протокол HTTPS не используется, сторонние файлы cookie отклоняются.| Необязательный, но если задан, требует использования протокола HTTPS. |`Set-Cookie: key=value; SameSite=None; Secure` |
...
В соответствии с обновленными ограничениями для SameSite браузер не добавляет файл cookie на веб-сайт, который уже прошел проверку подлинности, если ссылка взята с внешнего сайта. Убедитесь, что файлы cookie проверки подлинности помечены для межсайтового использования с помощью `SameSite=None; Secure` или доступен резервный вариант.

## <a name="android-system-webview"></a>Android System WebView

Android WebView — это системный компонент Chrome, который позволяет приложениям для Android отображать веб-содержимое. Хотя новые ограничения используются по умолчанию, начиная с Chrome 80, они не применяются к веб-представлениям сразу же. Они будут применены в будущем. В целях подготовки Android разрешает собственным приложениям задавать файлы cookie непосредственно через [API CookieManager](https://developer.android.com/reference/android/webkit/CookieManager).
