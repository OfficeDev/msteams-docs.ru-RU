...
>
> * Авторегистрация сервера Moodle с [помощью Microsoft Azure Active Directory (Azure AD)](https://azure.microsoft.com/services/active-directory/).
>
...
>
> Дополнительные данные о возможностях, которые предоставляет эта интеграция, см. в Microsoft Teams [Moodle](https://education.microsoft.com/resource/3dffb3a8).
...
Убедитесь, что перед установкой плагинов Microsoft 365 Moodle необходимо скачать следующее:
...
1. Вопишитесь на сервер Moodle в качестве администратора и  выберите администрирование [](https://docs.moodle.org/22/en/Settings_block) сайта из Параметры, расположенного в левой панели навигации.

1. Выберите **вкладку Plugins**, а затем установите **плагины.**

1. В разделе **Установите плагины из файла ZIP** выберите **Выберите файл**.
...
1. Выберите **администрирование сайта** из левой панели навигации, чтобы вернуться на панель мониторинга администрирования. Прокрутите вниз **к локальным плагинам** **и выберите ссылку Microsoft 365 интеграции**.
    ...
    > [!IMPORTANT]
    >
    > * Не Microsoft 365 страницу конфигурации плагинов Moodle на отдельной вкладке браузера, так как вам необходимо вернуться к этому набору страниц на протяжении всего процесса.  
    >
...
## <a name="2-configure-the-connection-between-the-microsoft-365-plugins-and-azure-ad"></a>2. Настройка подключения между плагинами Microsoft 365 и Azure AD
...
1. На странице Microsoft 365 интеграции выберите вкладку **Установки**.
...
1. Перейдите к unzipped каталогу, введя `cd .../.../Moodle-AzureAD-Powershell` `.../...` в текст путь к каталогу.
...
1. Далее необходимо добавить и `AppID` добавить `Key` в Microsoft 365 Плагины Moodle. Возвращайся на страницу администрирования плагинов, администрирование > плагинов > Microsoft 365 интеграции.

1. На **вкладке Установка** добавьте и `AppID` `Key` скопировали ранее, а затем выберите **Сохранить изменения**. После обновления страницы можно увидеть новый раздел **Выбор метода подключения**.

1. В **методе Выбор подключения** выберите почтовый ящик с меткой **Default** и снова выберите **сохранить изменения** .
...
13. Чтобы проверить [задания cron](https://docs.moodle.org/310/en/Cron) и выполнить их вручную для первого запуска, выберите ссылку  на страницу управления запланированными задачами в разделе **Синхронизация пользователей с azure AD**. В этом случае вы **перенабирались на страницу Запланированные** задачи.
    ...
    1. Если вы выберете для создания групп на основе существующих курсов, вы также можете запустить группы **пользователей Create в Microsoft 365** работе.
...
1. Возвращайся на страницу администрирования плагинов, администрирование **> плагинов > Microsoft 365 интеграции** и **выберите Teams Параметры** страницу.

1. На **Teams Параметры настроить** необходимые параметры, чтобы включить интеграцию Teams приложения.

    1. Чтобы включить **Подключение OpenID**, выберите ссылку Управление проверкой подлинности и выберите значок глаза в строке **OpenId Подключение**, если она серый.
    ...
    1. Чтобы включить внешние службы для Microsoft 365, выберите ссылку **Внешние** службы, а затем:  
        ...
        ✔ Выберите **Изменить** в строке **Moodle Microsoft 365 веб-служб**.
    ...
    1. Отредактировать разрешения пользователя, проверку подлинности, чтобы разрешить им создавать маркеры веб-службы.
        ...
        ✔ Выберите **ссылку пользователя На роль редактирования, которая была аутентификацией** .
...
1. **Скопируйте ID приложения и** вклеите его в поле **Бот-приложения на** странице **Параметры**.
...
После развертывания бота в Azure и настройки для беседы с сервером Moodle необходимо развернуть Microsoft Teams приложение. Для этого необходимо загрузить файл манифеста приложения, скачаемый с Microsoft 365 группы плагинов Moodle Параметры на предыдущем шаге.
...
4. Выберите пакет `manifest.zip` , который вы скачали ранее, и выберите **Сохранить**. Если вы не скачали пакет манифеста приложения, вы можете скачать с вкладки **Team Параметры** страницы конфигурации плагинов в Moodle.
...
Несмотря на то, что вкладки Moodle создаются вручную в Microsoft Teams, их можно создать автоматически, когда группы создаются из синхронизации курсов. Для этого необходимо настроить ID загруженного Microsoft Teams в Moodle.
...
1. В текстовом редакторе вклеить скопированные материалы. Он должен содержать URL-адрес, например `https://teams.microsoft.com/l/app/00112233-4455-6677-8899-aabbccddeeff`. Скопируйте последнюю часть URL-адреса, `00112233-4455-6677-8899-aabbccddeeff`например , который является ID Microsoft Teams приложения.

1. В Moodle откройте **вкладку Teams Moodle** на странице конфигурации Microsoft 365 moodle Plugins.

1. Вклеите ID приложения Microsoft Teams в поле ID приложения Moodle и сохраните изменения.
