...
Позволяет указать язык по умолчанию и указатели на другие языковые файлы. Подробности см. в разделе [Локализация](~/concepts/build-and-test/apps-localization.md).
...
> [!NOTE]
>  Функция searchUrl недоступна сторонним разработчикам. Если на вкладках требуется контекстно-зависимая информация для отображения соответствующего содержимого или для запуска потока проверки подлинности, см. статью [Получение контекста для вкладок Microsoft Teams](../../tabs/how-to/access-teams-context.md).
...
|`scopes`|массив элементов enum|1|✔|Указывает, предоставляет ли соединитель возможности в контексте канала в `team` или только для отдельных пользователей (`personal`). В настоящий момент поддерживается только область `team`.|
...
|`fetchTask`|Boolean|||Логическое значение, указывающее, нужно ли динамически получать модуль задачи. По умолчанию: **false**.|
...
Указывает, отображается ли панель заголовка вкладки при отрисовке личного приложения. По умолчанию: **false**.
...
|`groupchat`|Строка|||Если выбрана область установки `groupchat`, в этом поле указывается доступная по умолчанию емкость. Возможные варианты: `tab`, `bot` или `connector`.|
|`meetings`|Строка|||Если выбрана область установки `meetings`, в этом поле указывается доступная по умолчанию емкость. Возможные варианты: `tab`, `bot` или `connector`.|
...
|Имя| Тип|Максимальный размер|Обязательный |Описание|
|---|---|---|---|---|
|`type`|string||✔| Тип разрешения для определенного ресурса. Возможные варианты: `Application` и `Delegated`.|
