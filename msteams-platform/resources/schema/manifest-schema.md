...
|`scopes`|массив элементов enum|3|✔️|Указывает, предлагает ли бот функции в контексте канала в `team`, в групповом чате (`groupchat`) или только для отдельных пользователей (`personal`). Эти параметры не являются взаимоисключающими.|
...
|Имя| Тип| Максимальный размер | Обязательный | Описание|
|---|---|---|---|---|
|`items.scopes`|массив элементов enum|3|✔️|Указывает область, для которой действует список команд. Доступные варианты: `team`, `personal` и `groupchat`.|
...
|`scopes`|массив элементов enum|1|✔️|Указывает, предоставляет ли соединитель функции в контексте канала в `team` или только для отдельных пользователей (`personal`). В настоящий момент поддерживается только область `team`.|
...
|`canUpdateConfiguration`|Логическое|||Значение, указывающее, может ли пользователь обновлять конфигурацию расширения для обмена сообщениями. По умолчанию: **false**.|
...
|`parameters.name`|string|64 символа|✔️|Имя параметра в том виде, в котором оно отображается в клиенте. Имя параметра включено в запрос пользователя.|
...
> * Для правильной работы ваше приложение может учитывать уникальные свойства каждого типа канала. Чтобы включить вкладку для частных и общих каналов, см. статьи о [получении контекста в частных каналах](~/tabs/how-to/access-teams-context.md#retrieve-context-in-private-channels) и о [получении контекста в общих каналах](~/tabs/how-to/access-teams-context.md#retrieve-context-in-microsoft-teams-connect-shared-channels).
...
> [!NOTE]
> Если для `manifestVersion` свойства установлено значение 1.12, свойство разрешения несовместимо со старыми версиями (версия 1.11 или более ранняя) манифеста. Авторизация поддерживается для манифеста версии 1.12.
