...
description: В этом модуле вы узнаете о предстоящих и внесенных изменениях в API Бота, используемых для получения участников команд и чатов.
...
* К концу 2022 года боты не смогут заранее получать свойства `userPrincipalName` и `email` участников чата или группы. Боты должны использовать API Graph для получения необходимых сведений. Новый API `GetConversationPagedMembers` утратит возможность возвращать свойства `userPrincipalName` и `email`в конце 2022 года.

    > [!NOTE]
    > Для получения сведений рекомендуется использовать [API Graph](/graph/api/user-get?view=graph-rest-1.0&tabs=http&preserve-view=true#examples) с маркером доступа.
