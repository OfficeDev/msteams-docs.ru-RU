...
Сердцем расширения обмена сообщениями является веб-служба. Он определяет один маршрут, как правило `/api/messages`, для получения всех запросов. Если вы начинаете работу с нуля, у вас есть несколько вариантов на выбор.
...
* Если у вас есть подписка Azure (или вы хотите создать новую), вы можете зарегистрировать веб-службу вручную с помощью Microsoft Azure Portal. Создайте ресурс "Регистрация каналов ботов". Вы можете выбрать уровень бесплатных цен, так как сообщения из Microsoft Teams не учитываются в сумме допустимых сообщений в месяц.
...
На вкладке **Finish => test and distribute** можно скачать пакет приложения  (который включает манифест приложения, а также значки приложения) или установить пакет.

### <a name="create-your-app-manifest-manually"></a>Создание манифеста приложения вручную

Как и в ботах и вкладок, вы обновляете манифест приложения для приложения, чтобы включить свойства расширения обмена сообщениями.[](~/resources/schema/manifest-schema.md#composeextensions) Эти свойства регулируют, как отображается и ведет себя расширение обмена сообщениями в Microsoft Teams клиенте. Расширения обмена сообщениями поддерживаются начиная с v1.0 манифеста.

#### <a name="declare-your-messaging-extension"></a>Объявление расширения обмена сообщениями

Чтобы добавить расширение обмена сообщениями, включите новую структуру JSON верхнего уровня в манифест приложения с свойством `composeExtensions` . Для приложения создается одно расширение обмена сообщениями с до 10 командами.
...
1. **Метаданные**. При вызове расширения обмена сообщениями он может идентифицировать пользователя и клиента от `userId` и `tenantId`. `meetingId` может быть частью объекта `channelData`. Приложение может использовать запрос `userId` `meetingId`  API и `GetParticipant` для получения ролей пользователя.

1. **Тип команды**. Если в расширении сообщения используются команды, основанные на [действиях](../messaging-extensions/what-are-messaging-extensions.md#action-commands), оно должно выполнять проверку подлинности вкладки с одним [входом](../tabs/how-to/authentication/auth-aad-sso.md) .

1. **Пользовательский интерфейс**. Расширение обмена сообщениями должно выглядеть и вести себя так же, как и вне собрания.
