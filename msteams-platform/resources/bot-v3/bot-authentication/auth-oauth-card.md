...
Так как маркеры доступа являются конфиденциальной информацией, вы можете не хранить их во внешней службе. В этом случае вы можете по-прежнему реализовать собственную систему управления маркерами и интерфейс входа в Teams, как описано в остальных разделах Teams [проверки подлинности](~/resources/bot-v3/bot-authentication/auth-flow-bot.md).

## <a name="getting-started-with-oauthcard-in-teams"></a>Начало работы с OAuthCard в Teams

> [!NOTE]
> В этом руководстве используется пакет SDK Bot Framework версии 3. Реализацию версии 4 можно найти [здесь](/azure/bot-service/bot-builder-authentication?view=azure-bot-service-4.0&tabs=csharp&preserve-view=true). Вам по-прежнему `validDomains` потребуется создать манифест и включить token.botframework.com в раздел, так как в противном случае кнопка входа не откроет окно проверки подлинности. Используйте [App Studio для](~/concepts/build-and-test/app-studio-overview.md) создания манифеста.
...
1. Включите token.botframework.com `validDomains` в раздел манифеста приложения, так как Teams встраит Служба Bot страницы входа пользователя.
