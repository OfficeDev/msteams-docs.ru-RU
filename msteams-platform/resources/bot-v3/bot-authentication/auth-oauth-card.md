...
С помощью OAuthCard службы ботов Azure для бота Teams проще войти в пользователей и получить доступ к внешним поставщикам данных. Если вы уже внедрили auth и хотите перейти на что-то более простое, или если вы хотите добавить проверку подлинности в службу ботов впервые, OAuthCard может упростить ее.
...
В настоящее время существует ряд ограничений на использование OAuthCard. Такие законодательные акты перечислены ниже.
...
* Является беспробумным для конечных пользователей: выполните полный вход в режим работы прямо в Teams.
...
Так как маркеры доступа являются конфиденциальной информацией, вы можете не хранить их во внешней службе. В этом случае вы можете реализовать собственную систему управления маркерами и войти в Teams, как описано в остальных [темах](~/resources/bot-v3/bot-authentication/auth-flow-bot.md) проверки подлинности Teams.
...
1. Включите token.botframework.com в раздел манифеста приложения, так как Teams встраит страницу входа в `validDomains` службу ботов.
