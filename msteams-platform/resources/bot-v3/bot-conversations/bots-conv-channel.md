...
description: Описан сквозной сценарий общения с ботом в канале Microsoft Teams
keywords: сценарии Teams каналы беседа бот
...
Чат в каналах и групповой чат отличается от личного чата тем, что пользователю необходимо @упомянуть бота. Если бот используется в нескольких областях, таких как персональный, групповой чат или канал, необходимо определить область, из которой поступили сообщения бота, и обработать их соответствующим образом.

## <a name="designing-a-great-bot-for-channels-or-groups"></a>Проектирование качественного бота для каналов и групп

Бот, добавленный в группу, становится еще одним участником команды, и его можно @упоминать в ходе беседы. На самом деле боты получают сообщения только при @упоминании, поэтому другие сообщения беседы по каналу не отправляются боту.
...
Возможно, ваш бот уже полностью пригоден для работы во всех областях без дополнительной доработки. В Microsoft Teams не ожидается, что бот будет пригоден для всех областей, но следует убедиться, что бот предоставляет ценную для пользователя услугу в любой области, которую вы решите поддерживать. Подробнее об областях см. в разделе [Приложения в Microsoft Teams](~/concepts/build-and-test/app-studio-overview.md).
...
Дополнительные сведения о ботах, создающих сообщения в каналах, см. в разделе [Упреждающий обмен сообщениями для ботов](~/resources/bot-v3/bot-conversations/bots-conv-proactive.md) и,в частности, [Создание беседы по каналу](~/resources/bot-v3/bot-conversations/bots-conv-proactive.md#creating-a-channel-conversation).
...
При первом добавлении бота в группу или команду бывает полезно отправить приветственное сообщение с ботом всем пользователям. Приветственное сообщение должно содержать описание функциональных возможностей бота и пользы, которую он принесет участникам. В идеале сообщение должно также содержать команды для взаимодействия пользователя с приложением. Для этого убедитесь, что бот отвечает на сообщение `conversationUpdate` отправкой eventType `teamsAddMembers` в объекте `channelData`. Убедитесь, что идентификатор `memberAdded` является идентификатором приложения бота, так как то же событие отправляется при добавлении пользователя в команду. Подробнее см. в разделе [Добавление участника команды или бота](~/resources/bot-v3/bots-notifications.md#team-member-or-bot-addition).
...
Так как боты в группе или канале отвечают только в том случае, если они упомянуты в сообщении ("@*имя_бота*"), каждое сообщение, полученное ботом в канале группы, содержит его собственное имя, и вы должны гарантировать соответствующую обработку этого при анализе сообщений. Кроме того, боты могут извлекать посредством анализа имена других пользователей, упомянутых в сообщениях, и упоминать их в собственных сообщениях.
