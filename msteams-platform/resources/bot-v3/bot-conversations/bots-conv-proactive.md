---
title: Активный обмен сообщениями для ботов
description: Узнайте, как использовать активные сообщения для ботов в Microsoft Teams
ms.topic: conceptual
...
При создании упреждающих **сообщений** `MicrosoftAppCredentials.TrustServiceUrl`необходимо вызвать и `ConnectorClient` передать URL-адрес службы перед созданием используемого для отправки сообщения. Если этого не делать, ваше `401: Unauthorized` приложение получает ответ. Дополнительные сведения см. [в примерах ниже](#net-example-from-this-sample).

## <a name="best-practices-for-proactive-messaging"></a>Рекомендации по упреждающим сообщениям

Отправка активных сообщений — эффективный способ общения с пользователями. Однако с точки зрения пользователя сообщение отображается без причины. Если есть приветственное сообщение, это будет первый раз, когда пользователь взаимодействует с вашим приложением. Важно использовать эту функциональность и предоставить пользователю полную информацию, чтобы он понял назначение этого сообщения.
...
При использовании активной системы обмена сообщениями для отправки приветствия пользователю убедитесь, что с точки зрения пользователя сообщение будет непроизводимо. Если есть приветственное сообщение, это будет первый раз, когда пользователь взаимодействует с вашим приложением. Лучшие приветствия будут включать в себя:
...
* **Что произошло** с: Должно быть понятно, какой элемент/вещь был обновлен, чтобы вызвать уведомление.
* **Кто:** Кто, из-за чего было отправлено уведомление?
...
* Когда пользователь @mentioned [в разговоре канала](~/resources/bot-v3/bot-conversations/bots-conv-channel.md#-mentions) , бот является частью.
...
Иногда может потребоваться заблаговременное сообщение пользователям, которые ранее не устанавливали или не взаимодействовали с вашим приложением. Например, вы хотите использовать [корпоративный коммуникатор](~/samples/app-templates.md#company-communicator) для отправки сообщений всей организации. В этом сценарии вы можете использовать API Graph для упреждающего установки приложения для пользователей, `conversationUpdate` а затем кэшете необходимые значения из события, которое ваше приложение получит после установки.
...
[Дополнительные сведения см. в](/graph/api/userteamwork-post-installedapps?view=graph-rest-1.0&tabs=http&preserve-view=true) Graph документации по установке приложений для пользователей. Существует также пример [в .NET](https://github.com/microsoftgraph/contoso-airlines-teams-sample/blob/283523d45f5ce416111dfc34b8e49728b5012739/project/Models/GraphService.cs#L176).
...
Необходимо предоставить ИД пользователя и ИД клиента. Если вызов удался, API возвращается со следующим объектом ответа.
...
Бот, добавленный в команду, может публиковать сообщения на канале, создавая новые цепочки ответов. Если вы используете Node.js Teams SDK, `startReplyChain()`используйте , который дает вам полностью заполненный адрес с правильным ИД активности и ИД беседы. Если вы используете C#, см. пример ниже.
