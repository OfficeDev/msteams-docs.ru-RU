...
description: Узнайте об общих терминах, значениях и определениях, используемых в документации разработчика Microsoft Teams.
...
ms.locfileid: "68791442"
---
# <a name="glossary"></a>Глоссарий

Распространенные термины и определения, используемые в документации разработчика Microsoft Teams.
...
| [Разрешение приложения](../concepts/device-capabilities/browser-device-permissions.md#enable-apps-device-permissions) | Параметр в приложении Teams, чтобы включить разрешения устройства. Он доступен только в том случае, если в файле манифеста приложения объявлено, что приложению нужны разрешения устройства. <br> **См. также**[: Разрешения устройства](#d) |
| [Область приложения](../concepts/design/understand-use-cases.md#app-scope) | Область в Teams, где люди могут использовать ваше приложение. Приложения могут иметь одну или несколько областей, включая личную область, каналы, чаты и собрания. Приложение Teams может существовать в разных областях. |
...
| [Azure Active Directory](../tabs/how-to/authentication/auth-tab-aad.md) | Облачная служба управления идентификацией и доступом Майкрософт. Это помогает пользователям, прошедшим проверку подлинности, получать доступ к внутренним и внешним ресурсам. |
| [Проверка подлинности](../concepts/authentication/authentication.md) | Процесс проверки удостоверения пользователя для доступа к приложению. <br> **См. также**: [поставщики удостоверений](#i); [единый вход](#s) |
| [Поток проверки подлинности](../concepts/authentication/authentication.md) | Способ проверки подлинности пользователя в приложении. Для приложений Teams рекомендуется использовать единый вход (SSO) с помощью Azure Active Directory (AAD), но альтернативой является использование стороннего поставщика OAuth.|
...
| [Чат-бот](../bots/how-to/conversations/conversation-basics.md) | Бот также называется чат-ботом или ботом для общения. Это приложение, выполняющее простые и повторяющиеся задачи для пользователей, таких как отдел обслуживания клиентов или служба поддержки клиентов. <br> **См. также**: [бот для общения](#c) |
...
| [Секрет клиента](../bots/how-to/authentication/add-authentication.md) | Строка секрета, которую приложение использует для подтверждения своего удостоверения при запросе маркера. Кроме того, его можно назвать паролем приложения.|
...
| [CommandBox](../resources/schema/manifest-schema.md) | Тип контекста в манифесте приложения (`commandBox`), который можно настроить для вызова расширения для обмена сообщениями из командного поля Teams. |
...
|[Министерство обороны (DoD)](../concepts/app-fundamentals-overview.md#government-community-cloud)| Среды Министерства обороны обеспечивают соблюдение требований к безопасности Министерства обороны, Дополнения федеральных правил приобретения обороны (DFARS) и международных правил торговли оружием (ITAR).|
...
| [Среда разработчика](../toolkit/TeamsFx-multi-env.md#create-new-environment) | Тип среды разработки, которую по умолчанию создает набор средств Teams. Он представляет конфигурации удаленной или облачной среды. Проект может иметь несколько удаленных сред. Вы можете добавить в свой проект дополнительные среды разработчика с помощью набора средств Teams. <br>**См. также:** [среда](#e); [локальная среда](#l) |
| [Средства разработчика](../tabs/how-to/developer-tools.md) | Инструменты разработчика браузера используются для просмотра журналов консоли, просмотра или изменения сетевых запросов во время выполнения, добавления точек останова в код (JavaScript) и выполнения интерактивной отладки для приложения Teams. Эта функция доступна только для клиентов классической версии и Android после включения предварительной версии для разработчика. |
...
| Термин | Определение |
| --- | --- |
| [Интерфейс первого запуска](../concepts/design/design-teams-app-ui-templates.md)|Интерфейс первого запуска (FRE) — это знакомство пользователя с вашим продуктом. FRE помогает пользователям приступить к работе с функциями, функциями и преимуществами продукта и влияет на то, чтобы пользователи вернулись и продолжили использовать ваш продукт.|

## <a name="g"></a>G

| Термин | Определение |
| --- | --- |
|[Облако сообщества для государственных организаций (GCC)](../concepts/app-fundamentals-overview.md#government-community-cloud)| Среда GCC обеспечивает соответствие федеральным требованиям к облачным службам, включая FedRAMP High, дополнение к федеральным правилам приобретения обороны (DFARS), а также требования к уголовной юстиции и федеральным налоговым информационным системам (типы данных CJI и FTI).|
|[Облако сообщества для государственных организаций (GCC) High](../concepts/app-fundamentals-overview.md#government-community-cloud)|Высокие среды GCC обеспечивают соблюдение требований к безопасности Министерства обороны (DOD), Дополнения федеральных правил приобретения обороны (DFARS) и международных правил торговли оружием (ITAR).<br>**См. также**: [Министерство обороны (DoD)](#d)|
...
| Термин | Определение |
| --- | --- |
| [Поставщик удостоверений](../concepts/authentication/authentication.md) | Сущность, которая хранит и предоставляет учетные данные пользователю. Это также позволяет пользователям регистрироваться самостоятельно.  <br>**См. также**: [проверка подлинности](#a) |
