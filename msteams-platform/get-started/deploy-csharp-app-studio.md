...
description: Узнайте, как развертывать Microsoft Teams с помощью C# или .NET. в App Studio
...
1. Чтобы создать пакет приложений для Teams, выберите вкладку **редактора Манифеста** в **App Studio**:
    ...
    Пример поставляется с собственным манифестом и предназначен для создания пакета приложений при построении проекта. Файл manifest.json можно найти в Visual Studio в Манифесте под ```Microsoft.Teams.Samples.HelloWorld.Web```.

     В Visual Studio файл manifest.json находится в соответствии с **Манифестом** в `Microsoft.Teams.Samples.HelloWorld.Web`. Этот шаг описывается следующим образом:  
...
После установки приложения в Teams настроить его, чтобы отобразить вкладку, которую необходимо загрузить.
...
- Выберите канал в команде, в которой вы зарегистрировали приложение и введите.`@your-bot-name` Этот тип сообщения называется **упоминанием\@**. Бот отвечает на любое сообщение, которое вы отправляете.
