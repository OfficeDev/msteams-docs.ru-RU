...
1. Откройте Teams набор средств и выберите значок Teams на боковой панели:
...
1. В разделе **Выбор возможностей** убедитесь, что **вкладка** выбрана и выберите **ОК.**
...
1. В разделе **Тип размещения в Frontend** выберите **Azure.**
...
1. В разделе **Облачные ресурсы** выберите **ОК**.  В этом руководстве вам не понадобятся дополнительные облачные ресурсы.
...
1. В качестве размещения на клиенте выберите **Azure**. Не добавляйте облачные ресурсы.
...
После настройки Teams набор средств проекта у вас есть компоненты для создания базового личного приложения для Teams. Каталоги и файлы проекта отображаются в области проводника Visual Studio Code.
...
- Веб-API позволяет выполнять задачи проверки подлинности, действуя в качестве прокси-сервера между приложением и Azure Active Directory. Доступ к нему можно получить по URL `https://localhost:5000`.
...
1. В Visual Studio Code нажмите клавишу **F5**, чтобы запустить приложение в режиме отладки.
   ...
   Впервые для локального запуска вам будет предложено установить сертификат для локального отлаговорки. нажмите кнопку Продолжить.
     :::image type="content" source="../assets/images/teams-toolkit-v2/certificate-prompt.png" alt-text="Снимок экрана: запрос на установку SSL-сертификата, чтобы разрешить Teams загрузку вашего приложения из localhost.":::
...
1. Во входе в учетную запись M365 при запросе.
...
* Запускает локализованную работу backend приложения с помощью основных средств [Azure Function.](/azure/azure-functions/functions-run-local?#start)
* Запускает локальное локальное хостинг приложения.
* Начинается Microsoft Teams в веб-браузере с командой Teams, чтобы загрузить приложение из `https://localhost:3000/tab` . Это URL-адрес, зарегистрированный в манифесте приложения.
...
* При настройке в приложении используется множество служб Azure, включая Службу приложений Azure и служба хранилища Azure.
