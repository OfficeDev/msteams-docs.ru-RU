...
1. Выберите **Сохранить** и выбрать **view в Teams,** чтобы быстро протестировать сцену в Microsoft Teams.
    ...
    * Выбор **представления в Teams** автоматически создает пакет приложений, который appmanifest.jsза кулисами. Вы можете перейти  **к приложениям** из меню и получить доступ к автоматически созданному пакету приложений.
...
Необязательный пакет сцены извлекают, выбрав **экспорт** из меню **Сохранить** выпадающий. Загружается **.zip,** то есть пакет сцены. Пакет сцены включает scene.jsи активы PNG, используемые для создания сцены. Пакет сцены просматривается для включения других изменений:
...
Scene.jsвместе с изображениями указывают точное расположение сидений. Сцена состоит из изображений bitmap, спрайтов и прямоугольников для вложения видео участников. Эти спрайты и ящики участников определяются в системе координат мира. X-axis указывает направо, а Y-оси — вниз.
...
В примере scene.jsследующий код:
...
Теперь, когда вы прошли через пример scene.js, вы можете активировать настраиваемые сцены Режима совместно, чтобы участвовать в сценах.
