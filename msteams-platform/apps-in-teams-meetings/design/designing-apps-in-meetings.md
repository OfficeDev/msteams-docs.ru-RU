...
На собрании пользователи могут выбрать **дополнительные** :::image type="icon" source="../../assets/icons/teams-client-more.png"::: >  добавления **приложения** и выбрать нужное приложение.
...
|2|**Имя приложения**|
|3|**Заглавная**. Включает имя приложения.|
|4|**Кнопка закрыть**: отклонять вкладку. Всегда используйте значок верхнего правого закрытия вместо действия в подножке.|
|5|**Панели уведомлений**. Оповещения об ошибках отображаются непосредственно под заголовком и выдвигают остальную часть контента iframe вниз на 20 пикселей.|
...
Вы можете разрешить пользователям обмениваться некоторым или всем контентом приложения на стадии собрания и взаимодействовать с ними. Вот примеры того, как люди могут использовать эту функцию во время собрания:
...
Приложения, общие на этапе собрания, занимают то же место, что и общий экран. Этап также переориентирован для всех участников собрания одинаково.

> [!NOTE]
> В настоящее время мобильные пользователи не могут обмениваться контентом приложений на стадии собрания. Однако они могут видеть содержимое, совместное с рабочего стола.
...
:::image type="content" source="~/assets/images/apps-in-meetings/shared-meeting-stage-edit-review-component.png" alt-text="В примере показан компонент панели мониторинга, который просматривается на общем этапе собрания." border="false":::
...
**Доска**: рисуйте и задумайте вместе на общем холсте.
...
### <a name="anatomy-share-all-app-content-to-a-meeting"></a>Анатомия. Поделитесь всем контентом приложения на собрании

:::image type="content" source="~/assets/images/apps-in-meetings/shared-meeting-stage-anatomy.png" alt-text="На изображении показана анатомия дизайна общей стадии собраний, когда все содержимое приложения является общим." border="false":::

|Счетчик|Описание|
|----------|-----------|
|1|**Значок приложения**. Выделенная значок указывает, что вкладка приложения на собрании открыта.|
|2|**Кнопка Share to meeting**: точка входа, чтобы поделиться приложением с собранием. Отображает, если настроить приложение для использования общей стадии собраний.|
|3|**Атрибуция презентщика**: отображает имя участника, который поделился приложением.|
|4|**iframe.** Отображает содержимое приложения.|
|5|**Остановка доступа к кнопке**: перестает делиться приложением со стадией собрания. Отображает только участника, который запустил долю.|

### <a name="anatomy-share-specific-app-content-to-a-meeting"></a>Анатомия: Доля определенного контента приложения на собрании

:::image type="content" source="~/assets/images/apps-in-meetings/shared-meeting-stage-anatomy-component.png" alt-text="На изображении показана анатомия дизайна общей стадии собраний, когда общий доступ к только определенному контенту приложения." border="false":::

|Счетчик|Описание|
|----------|-----------|
|1|**Значок приложения**. Выделенная значок указывает, что вкладка приложения на собрании открыта.|
|2|**Кнопка Share to meeting**: точка входа, чтобы поделиться приложением с собранием. Для последовательного использования всегда используйте стандартный значок Teams обмена. **Share to meeting** — это рекомендуемый текст по умолчанию, но вы также можете настроить его для случаев использования. Например, **играть вместе** для игрового приложения или **смотреть вместе** для видео-приложения. В любом случае, убедитесь, что действие создаст общий интерактивный опыт со всеми на собрании.|
|3|**Атрибуция презентщика**: отображает имя участника, который поделился приложением.|
|4|**iframe.** Отображает содержимое приложения.|
|5|**Остановка доступа к кнопке**: перестает делиться приложением со стадией собрания. Отображает только участника, который запустил долю.|
...
* **Боковая** панель. Пользователь может открыть боковую панель в любое время во время собрания, чтобы пообщаться, просмотреть список или использовать приложение (например, вкладку в собрании). Этап динамически перестановки, когда панель открыта.
...
Цвета, конфликтующие с средой собраний, могут отвлекать и казаться менее родными для Teams. Узнайте о рампе [Teams цвета,](https://developer.microsoft.com/fluentui#/styles/web/colors/products) включая нейтральные темы вызовов.
