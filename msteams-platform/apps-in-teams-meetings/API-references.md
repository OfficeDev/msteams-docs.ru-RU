...
|[**Отправка уведомления о собрании**](#send-an-in-meeting-notification)| Предоставление сигналов собраний с помощью существующего API уведомлений о беседе для чата пользователя-бота и позволяет уведомлять пользователя о действии, отображаемом в уведомлении о собрании. |MSBF SDK|
...
|[**Получите события Teams в режиме реального времени**](#get-real-time-teams-meeting-events-api)|Извлечение событий собраний в режиме реального времени, таких как фактическое время начала и окончания.| MSBF SDK|
...
|---|---|----|---|
|**meetingId**| Строка | Да | Идентификатор собрания доступен с помощью Bot Invoke и Teams клиентской SDK.|
...
## <a name="send-an-in-meeting-notification"></a>Отправка уведомления о собрании

Все пользователи на собрании получают уведомления, отправленные с помощью полезной нагрузки уведомления на собрании. Полезное уведомление на собрании вызывает уведомление на собрании и позволяет предоставлять сигналы собраний, которые доставляются с помощью существующего API уведомлений о беседе для чата пользователя-бота. Вы можете отправить уведомление на собрании в зависимости от действий пользователя. Полезной нагрузки можно получить через службы bot.
...
> * URL-адрес — это страница, загружаемая `<iframe>` как в уведомлении о собрании. Домен должен быть в массиве приложений `validDomains` в манифесте приложения.
...
| **401** | Несанкционированный. Плохой или просроченный маркер. Если вы получите эту ошибку, сгенерировать новый URL-адрес КОРЗИНЫ в Teams. |
...
|Значение|Тип|Обязательный|Описание|
|---|---|----|---|
|**callback**| Строка | Да | Callback содержит два параметра: ошибка и результат. Ошибка *может* содержать ошибку типа *SdkError* или null при успешном совместном окне. Результат *может* содержать истинное значение в случае успешной доли или нулевую при сбойе.|
