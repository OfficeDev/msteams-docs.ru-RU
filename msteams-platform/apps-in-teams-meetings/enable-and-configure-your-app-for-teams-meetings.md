...
| **meetingStage** | Приложение из области `meetingSidePanel` можно использовать на стадии собрания. Вы не можете использовать это приложение на мобильных устройствах. |
...
Общая стадия собрания позволяет участникам собраний взаимодействовать с контентом приложения и сотрудничать с ним в режиме реального времени.

Необходимый контекст находится `meetingStage` в манифесте приложения. Обязательным условием является учет `meetingSidePanel` контекста, который включает **Share** в `meetingSidePanel` .

![Share to stage during meeting experience](~/assets/images/apps-in-meetings/share_to_stage_during_meeting.png)

Чтобы включить общий этап собрания, настройте манифест приложения следующим образом:
...
|----------------|-----------------|--------------|----------------|-----------|
| Приложение для собраний | Демонстрирует, как использовать приложение Генератор маркеров собраний для запроса маркера. Маркер создается последовательно, чтобы каждый участник имел справедливую возможность участвовать в собрании. Маркер полезен в таких ситуациях, как scrum meetings и Q&A. | [Просмотр](https://github.com/OfficeDev/microsoft-teams-sample-meetings-token) |
