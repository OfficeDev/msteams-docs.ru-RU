...
Диалоговое окно на собрании используется для вовлечения участников во время собрания и сбора сведений или отзывов во время собрания. Чтобы вызвать [`NotificationSignal`](API-references.md#notificationsignal-api) уведомление о пузыре, используйте API. В качестве полезной нагрузки запроса уведомлений включайте URL-адрес, на котором будет хозяйствовать контент.
...
> [!NOTE]
> * Необходимо вызвать функцию [submitTask()](../task-modules-and-cards/task-modules/task-modules-bots.md#submit-the-result-of-a-task-module) для автоматического увольнения после действия пользователя в веб-представлении. Это требование для отправки приложения. Дополнительные сведения см. [Teams модулем задач SDK](/javascript/api/@microsoft/teams-js/microsoftteams.tasks?view=msteams-client-js-latest#@microsoft-teams-js-microsoftteams-tasks-submittask&preserve-view=true).
...
|Боковая панель собраний | Пример приложения, чтобы показать, как добавить повестку дня в боковой панели собрания | [Просмотр](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/meetings-sidepanel/csharp) |-|
