...
Во время собрания вы можете `meetingSidePanel` использовать диалоговое окно или диалоговое окно на собрании для создания уникальных приложений.
...
Диалоговое окно на собрании используется для вовлечения участников во время собрания и сбора сведений или отзывов во время собрания. Чтобы вызвать уведомление о пузыре [, используйте API SendNotificationSignal](API-references.md#send-notification-signal-api) . В качестве полезной нагрузки запроса уведомлений включайте URL-адрес, на котором будет хозяйствовать контент.

Диалоговое окно на собрании не должно использовать модуль задач. Модуль задач не вызывается в чате собраний. Url-адрес внешнего ресурса используется для отображения пузыря контента на собрании. Этот метод можно использовать для `submitTask` отправки данных в чате собраний.
...
> * Если вы хотите, чтобы ваше приложение поддержало анонимных пользователей, необходимо использовать метаданные запроса в объекте, `from.id` `from` `from.aadObjectId` а не запрашивать метаданные. `from.id`является ИД пользователя `from.aadObjectId` и является Azure Active Directory (AAD) пользователя. Дополнительные сведения см. в [таблицах с](../task-modules-and-cards/task-modules/task-modules-tabs.md) использованием модулей задач и [созданием и отправкой модуля задач](../messaging-extensions/how-to/action-commands/create-task-module.md?tabs=dotnet#the-initial-invoke-request).
...
Если вы хотите, чтобы ваше приложение поддержало анонимных пользователей, необходимо использовать метаданные запроса в объекте, `from.id` `from` `from.aadObjectId` а не запрашивать метаданные. `from.id`является пользовательским ИД `from.aadObjectId` и является Azure Active Directory ИД пользователя. Дополнительные сведения см. в [таблицах с](../task-modules-and-cards/task-modules/task-modules-tabs.md) использованием модулей задач и [созданием и отправкой модуля задач](../messaging-extensions/how-to/action-commands/create-task-module.md?tabs=dotnet#the-initial-invoke-request).
...
* Следуйте [пошаговом руководстве](../sbs-meeting-token-generator.yml) по генерации маркера собраний в Teams собрании.
* Следуйте [пошаговом](../sbs-meetings-sidepanel.yml) руководстве для создания боковогопанеля собраний в Teams собрании.
* Следуйте [пошаговом](../sbs-meetings-stage-view.yml) руководстве, чтобы создать представление стадии собрания в Teams собрании.
