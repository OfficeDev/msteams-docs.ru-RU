...
1. Пользователь может просмотреть сообщение адаптивной карточки, прежде чем бот вставит его в канал. Если приложение не является участником канала, `Send` выберите, чтобы добавить его.
    ...
    >
1. После выбора пользователем `Send`бот отправляет сообщение в канал.
