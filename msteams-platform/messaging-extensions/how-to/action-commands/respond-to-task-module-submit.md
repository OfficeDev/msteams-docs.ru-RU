...
После отправки модуля задач `composeExtension/submitAction` веб-служба получает сообщение вызова с командным ИД и значениями параметров. У приложения есть пять секунд для ответа на вызов, в противном случае пользователь получает сообщение об ошибке, не достигаемом **приложения, и** любой ответ на вызов игнорируется Teams клиентом.

У вас есть следующие варианты ответа:

* Нет ответа. Используйте действие отправки, чтобы вызвать процесс во внешней системе и не предоставлять пользователю никаких отзывов. Это полезно для длительных процессов и предоставлять обратную связь поочередно. Например, вы можете дать обратную связь с помощью [проактивного сообщения](~/bots/how-to/conversations/send-proactive-messages.md).
...
* [Адаптивная карта от бота](#bot-response-with-adaptive-card): вставьте адаптивную карту непосредственно в беседу.
...
Для проверки подлинности или настройки после завершения процесса пользователь вызывает повторное вызов в веб-службу. В следующей таблице показано, какие `commandContext` типы ответов доступны в зависимости от расположения ссылок расширения обмена сообщениями:
...
Если приложение содержит разговорный бот, установите бот в беседе и загрузив модуль задач. Бот полезен для получения дополнительного контекста для модуля задач. Чтобы установить разговорный бот, см. [запрос на установку беседного бота](create-task-module.md#request-to-install-your-conversational-bot).
...
Это пример получаемого объекта JSON. Параметр `commandContext` указывает, откуда было вызвано расширение обмена сообщениями. Объект `data` содержит поля формы в качестве параметров и значения, которые пользователь представил. Объект JSON выделяет наиболее релевантные поля.
...
Наиболее распространенный способ ответа на запрос `composeExtension/submitAction` — это карта, вставленная в область составить сообщение. Пользователь передает карточку в беседу. Дополнительные сведения об использовании карт см. в [карточках и действиях карт](~/task-modules-and-cards/cards/cards-actions.md).
...
Вы можете выбрать, чтобы ответить на событие `submitAction` с помощью дополнительного модуля задач. Это полезно, если необходимо:

* Сбор больших объемов информации.
* Динамически измените коллекцию сведений на основе ввода пользователя.
* Проверка сведений, представленных пользователем, и повторное отправка формы сообщением об ошибке, если что-то не так.

Метод ответа такой же, как [и ответ на начальное `fetchTask` событие](~/messaging-extensions/how-to/action-commands/create-task-module.md). Если вы используете SDK Bot Framework, запускается одно и то же событие для отправки обоих действий. Чтобы сделать эту работу, необходимо добавить логику, которая определяет правильный ответ.

## <a name="bot-response-with-adaptive-card"></a>Ответ бота с адаптивной картой

> [!NOTE]
> Обязательным условием получения ответа `bot` бота с адаптивной картой является то, что необходимо добавить объект в манифест приложения и определить требуемую область для бота. Используйте тот же ID, что и расширение обмена сообщениями для бота.

Вы также можете ответить на `submitAction` сообщение, вставив сообщение с адаптивной картой в канал с помощью бота. Пользователь может предварительно просмотреть сообщение перед его отправкой. Это полезно в сценариях, в которых вы собираете информацию от пользователей перед созданием ответа адаптивной карты или при обновлении карты после ее взаимодействия.
...
1. После отправки модуля задач приложение использует сведения, `botMessagePreview` предоставленные для создания опроса в качестве адаптивной карты, и отправляет его в качестве ответа клиенту.
1. Затем пользователь может просмотреть сообщение адаптивной карты, прежде чем бот вставляет его в канал. Если приложение не является участником канала, выберите `Send` добавить его.

    > [!NOTE]
    >
    > * Пользователи также могут выбрать сообщение `Edit` , которое возвращает их в исходный модуль задач.
...
Модуль задач должен отвечать на начальное `composeExtension/submitAction` сообщение предварительным просмотром карты, которую отправляет бот каналу. Пользователь может проверить карту перед отправкой и попытаться установить бот в беседе, если бот еще не установлен.
...
> [!NOTE]
>
> * Необходимо `activityPreview` содержать действие с `message` одним вложением адаптивной карты. Значение `<< Card Payload >>` — это местоодатель для карточки, которая будет отправляться.
...
Расширение обмена сообщениями должно отвечать на два новых `composeExtension/submitAction` типа вызова, где `value.botMessagePreviewAction = "send"`и `value.botMessagePreviewAction = "edit"`.
...
Если пользователь перед отправкой редактирует карточку, выбрав **edit**, вы получите вызов `composeExtension/submitAction` с `value.botMessagePreviewAction = edit`помощью . Ответьте, вернув отправленный модуль задач в ответ на начальный `composeExtension/fetchTask` вызов, который начал взаимодействие. Это позволяет пользователю запустить процесс, повторно внося исходные сведения. Используйте доступные сведения для обновления модуля задач, чтобы пользователю не было необходимости заполнять всю информацию с нуля.
Дополнительные сведения об ответе на начальное событие `fetchTask` см. в [ответе на начальное `fetchTask` событие](~/messaging-extensions/how-to/action-commands/create-task-module.md).
...
Вы получаете новое сообщение `composeExtension/submitAction` , аналогичное следующему:
...
Чтобы использовать атрибутику пользователя в командах, `OnBehalfOf` необходимо добавить объект упоминания `ChannelData` `Activity` в полезной нагрузке, которая отправляется в Teams.
...
|`mri`|String|Идентификатор ресурса сообщений (MRI) человека, от имени которого отправляется сообщение. Имя отправитель сообщения будет отображаться как "\<user\> через \<bot name\>".|
