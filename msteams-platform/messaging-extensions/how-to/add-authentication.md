...
1. Если у пользователя нет проверки подлинности, `auth` `openUrl` отправьте ответ с предложенным действием, включая URL-адрес проверки подлинности.
...
#### <a name="response-example-for-a-sign-in-action"></a>Пример ответа для знака в действии
...
> * Размер всплывающих данных проверки подлинности можно определить, включив параметры строки запроса ширины и высоты. `Value = $"{_siteUrl}/searchSettings.html?settings={escapedSettings}",`
...
Как и в других встроенных Microsoft Teams, коду в окне необходимо сначала позвонить`microsoftTeams.initialize()`. Если код выполняет поток OAuth, вы можете передать Teams пользователя в окно, которое затем передает его в URL-адрес для регистрации OAuth.
