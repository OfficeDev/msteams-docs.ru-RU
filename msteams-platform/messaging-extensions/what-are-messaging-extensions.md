...
author: surbhigupta
description: Обзор расширений для сообщений на платформе Microsoft Teams
...
Расширение для сообщений состоит из веб-службы, которую вы размещаете, и манифеста приложения, который определяет, откуда в клиенте Microsoft Teams вызывается ваша веб-служба. Веб-служба пользуется схемой обмена сообщениями Bot Framework и протоколом безопасной связи, поэтому вам требуется зарегистрировать веб-службу в качестве бота в Bot Framework.
...
![команда поиска расширения для сообщения](~/assets/images/search-extension.png)
...
Веб-служба вызывается при вставке URL-адреса в область создания сообщения. Эта функция называется развертыванием ссылок. Вы можете подписаться на получение вызова, когда URL-адреса, содержащие определенный домен, вставляются в область создания сообщений. Веб-служба может "развернуть" URL-адрес в подробную карточку, предоставляющую больше информации, чем стандартная карточка предварительного просмотра веб-сайта. Вы можете добавить кнопки, чтобы пользователи могли действовать немедленно, не выходя из клиента Microsoft Teams.
На следующих изображениях показана функция развертывания ссылок при вставке ссылки в расширение для сообщений:

![развернуть ссылку](../assets/images/messaging-extension/unfurl-link.png)
