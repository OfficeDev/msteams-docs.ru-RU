...
* **Расширение для сообщений (команды действий).** Расширения для сообщений позволяют выполнять определенные действия с содержимым сообщения. Выбор действия открывает модуль задач.
* **Расширение для сообщений (контекст окна создания сообщения).** В окна создания сообщения можно создать расширение для сообщений, чтобы вместо обычного всплывающего окна открывать модуль задач. Зарезервируйте модули задач для сложных взаимодействий, таких как заполнение формы.

## <a name="anatomy"></a>Структура

Модули задач предоставляют гибкую рабочую область для взаимодействия с размещенными приложениями. Они построены с использованием технологии iframe (для рабочего стола) или веб-представления (для мобильных устройств), чтобы можно было проектировать модули задач с помощью шаблонов пользовательского интерфейса (рекомендуется) или с нуля.
...
Не помещайте сообщение об ошибке во всплывающее диалоговое окно поверх модуля задач. Это сбивает с толку пользователя.
