---
title: Выбор людей в адаптивных картах
...
ms.locfileid: "62062562"
---
# <a name="people-picker-in-adaptive-cards"></a>Выбор людей в адаптивных картах
...
|Свойство |Тип |Обязательный |Описание|
|--|--|--|--|
|**набор данных** |Строка |Да |Тип данных, которые необходимо получать динамически.|   
...
Вы можете использовать `Action.Submit` или `Action.Execute` отправлять выбранные данные в бот. Полезной нагрузкой, полученной на боте, является список AAD или ID, предоставленных `invoke` в статическом списке.
В подборщике людей, когда пользователь выбран в области управления, его значение `AAD ID` отослано обратно. Строка `AAD ID` и уникально идентифицирует пользователя в каталоге.

Формат значения, представленного боту, зависит от значения `isMultiSelect` свойства:

|значение `isMultiSelect`|Формат|
|--|--|
|false _(один выбор)_|<selected_AAD_ID>|
|true _(multi select)_|<selected_AAD_ID_1>,<selected_AAD_ID_2> <selected_AAD_ID_3>|  

С помощью предварительного предварительного выборщика людей `AAD ID` соответствующий пользователь. 
...
* Для одного пользователя в области управления укажите пользователя `AAD ID` в качестве `value` . 
* Для нескольких пользователей, таких как `isMultiSelect` `true` это, укажите строку s, разделенную `AAD ID` запятой.  
