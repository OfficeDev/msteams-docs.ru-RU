...
* Teams или используемые боты `Action.Submit`— подход, который откладывает фактическую модель связи на базовый канал.
* Outlook используется для `Action.Http` взаимодействия со внутренней службой, явно указанной в полезных данных адаптивной карточки.
...
Теперь вы можете отправлять одинаковые карточки как в Teams, так и Outlook и синхронизировать их друг с другом с помощью базового бота. Любые действия, выполняемые на любой платформе, отражаются на другой с помощью этой сборки один раз *,* разверните в любом месте (универсальные действия для адаптивных карточек).

На следующем рисунке показаны универсальные действия для адаптивных карточек Teams и Outlook:
...
Сегодня каждый пользователь в Teams или канале видит одно и то же представление и действия кнопки на адаптивной карточке. Однако в некоторых сценариях некоторым пользователям необходимо действовать по-разному и иметь доступ к разным сведениям в одном чате или канале.
