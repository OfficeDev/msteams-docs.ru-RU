...
* Office 365 Карты соединителя: Markdown и ограниченный HTML поддерживаются в Office 365 соединителях в текстовых полях.
...
> [!NOTE]
> * [Элементы мультимедиа в](https://adaptivecards.io/explorer/Media.html) настоящее время не поддерживаются в адаптивных картах на Teams платформе.
...
### <a name="aad-object-id-and-upn-in-user-mention"></a>AAD Object ID и UPN в упоминаемом пользователем 

Teams платформа позволяет упоминать пользователей с их AAD Object ID и имя принципа пользователя (UPN), в дополнение к существующим именем упоминания. Боты с адаптивными картами и соединителями с входящие веб-окки поддерживают два удостоверения упоминания пользователя. 

В следующей таблице описываются недавно поддерживаемые пользовательские ID::

|ID  | Вспомогательные возможности |   Описание | Пример |
|----------|--------|---------------|---------|
| ID объекта AAD | Бот, соединитель |  ID объекта пользователя AAD |  49c4641c-ab91-4248-aebb-6a7de286397b |
| Имя участника-пользователя | Бот, соединитель | UPN пользователя AAD | john.smith@microsoft.com |

#### <a name="user-mention-in-bots-with-adaptive-cards"></a>Упоминание пользователя в ботах с адаптивными картами 

Боты поддерживают упоминание пользователя с AAD Object ID и UPN в дополнение к существующим ID. Поддержка двух новых удостоверений доступна в ботах для текстовых сообщений, тела адаптивных карт и ответа на расширение обмена сообщениями. Боты поддерживают указанные в беседе и сценариях `invoke` ID-упоминания. Пользователь получает уведомление о канале активности при @mentioned с ID-данными. 
...
![Упоминание пользователя в боте с адаптивной картой](~/assets/images/authentication/user-mention-in-bot.png)

#### <a name="user-mention-in-incoming-webhook-with-adaptive-cards"></a>Упоминание пользователя в входящих веб-пользователях с адаптивными картами 

Входящие веб-пользователи начинают поддерживать упоминание пользователей в адаптивных картах с AAD Object ID и UPN.

> [!NOTE]    
> * Включить упоминание пользователя в схеме входящих веб-ок для поддержки AAD Object ID и UPN. 
> * Изменения пользовательского интерфейса и UX не требуются для упоминаний пользователей с AAD Object ID и UPN.      
...
![Упоминание пользователя в входящих веб-пользователях](~/assets/images/authentication/user-mention-in-incoming-webhook.png)
...
# <a name="markdown-format-for-office-365-connector-cards"></a>[Формат markdown для Office 365 соединители](#tab/connector-md)
...
* Office 365 Карты соединители. Ограниченное разметка и форматирование HTML поддерживаются в Office 365 соединителях.
...
# <a name="html-format-for-office-365-connector-cards"></a>[ФОРМАТ HTML для Office 365 соединителя](#tab/connector-html)
