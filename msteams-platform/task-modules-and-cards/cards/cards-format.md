...
Поддержка форматирования различается между различными типами карт, а отрисовка карты может незначительно отличаться между настольным компьютером и мобильными Teams клиентами, а также Teams в настольном браузере.

Вы можете включить в линию изображение с любой Teams карточкой. Изображения форматированы как , или файлы и не должны превышать  `.png` `.jpg` `.gif` 1024 ×1024 px или 1 МБ. Анимированный GIF не поддерживается официально. Дополнительные сведения см. в [справке Карточки.](./cards-reference.md#inline-card-images)

## <a name="formatting-cards-with-markdown"></a>Форматирование карт с помощью Markdown

Существует два типа карт, поддерживают Markdown в Teams:
...
> * **Карты соединителя O365:** Markdown и ограниченный HTML поддерживаются Office 365 соединителями в текстовых полях.
...
Форматирование несколько отличается между настольным компьютером и мобильными версиями Teams.
...
[Текстовые функции в адаптивных картах](/adaptive-cards/create/textfeatures) Указанные в этом разделе функции даты и локализации не поддерживаются в Teams.
...
* Объект внутри свойства в контенте карточки, который включает Teams пользователя, `mention` `msteams` указанный пользователем.
...
> [!NOTE]
> Для Teams мобильных приложений по умолчанию доступны функции представления сцен для изображений в адаптивных картах, и пользователи смогут просматривать изображения адаптивных карт на этапе просмотра, просто нажав на изображение, независимо от того, присутствует атрибут или `allowExpand` нет.
...
Эти скриншоты были созданы с Teams AppStudio, где текстовое свойство карты героя было задано следующей строке. Вы можете протестировать форматирование в собственных картах, изменяя этот код.
