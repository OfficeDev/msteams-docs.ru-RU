...
* [**Используйте API Graph Microsoft:**](#use-the-graph-apis) Этот метод работает для ботов во всех Microsoft Teams области:
  * `personal`
  * `channel`
  * `groupchat`

* [**Используйте API Teams бота:**](#use-the-teams-bot-apis) Это только файлы поддержки в `personal` контексте.

## <a name="use-the-graph-apis"></a>Использование Graph API

Публикация сообщений с вложениями карт, которые относятся к существующим SharePoint файлам, с Graph API для OneDrive [и SharePoint](/onedrive/developer/rest-api/). Чтобы использовать Graph API, получите доступ к следующему через стандартный поток авторизации OAuth 2.0:

* Папка OneDrive и `personal` `groupchat` файлы пользователя.
* Файлы в канале группы для `channel` файлов.

Graph API работают во всех Teams области. Дополнительные сведения см. в [сообщении отправки вложений в файл сообщений чата.](/graph/api/chatmessage-post?view=graph-rest-beta&tabs=http#example-4-file-attachments&preserve-view=true)
