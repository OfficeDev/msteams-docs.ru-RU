...
Отправлять сообщения с вложениями карт, которые относятся к существующим файлам SharePoint, используя Graph API для OneDrive [и SharePoint](/onedrive/developer/rest-api/). Чтобы использовать Graph API, получите доступ к следующему через стандартный поток авторизации OAuth 2.0:
...
| Свойство | Назначение |
| --- | --- |
| `downloadUrl` | OneDrive URL-адрес для получения содержимого файла. Пользователь может выдавать прямо из `HTTP GET` этого URL-адреса. |
...
Чтобы загрузить файл пользователю:
...
После отправки файла в OneDrive отправьте пользователю сообщение подтверждения. Сообщение должно содержать следующее `FileCard` вложение, которое пользователь может выбрать для предварительного просмотра или открытия в OneDrive или локальной загрузки:
...
В следующем примере кода показано, как получить согласие на файл и загрузить файлы Teams от бота:
