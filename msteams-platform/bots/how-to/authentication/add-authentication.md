...
- **Как развернуть бот в Azure и связать его с поставщиком удостоверений**. Поставщик выпускает маркер на основе учетных данных для входа пользователя. Бот может использовать маркер для доступа к ресурсам, таким как почтовая служба, для которых требуется проверка подлинности. Дополнительные сведения см. в описании  [потока проверки подлинности Microsoft Teams для ботов](auth-flow-bot.md).
...
1. Для параметра **Тип приложения** выберите **Мультитенантное** в разделе **Идентификатор приложения Майкрософт**.
   ...
   :::image type="content" source="../../../assets/images/adaptive-cards/multi-tenant.png" alt-text="На этом снимке экрана показано, как выбрать несколько клиентов для Microsoft AppID.":::

1. Выберите **Проверить и создать**.

   :::image type="content" source="../../../assets/images/adaptive-cards/create-azure-bot.png" alt-text="На этом снимке экрана показано, как создать бот Azure.":::

1. Если проверка пройдена, нажмите **Создать**.

    Подготовка службы бота займет несколько секунд.

   :::image type="content" source="../../../assets/images/adaptive-cards/validation-pane.png" alt-text="На этом снимке экрана показано, как проходит проверка бота Azure.":::

1. Выберите пункт **Перейти к ресурсу**. Бот и связанные ресурсы указаны в группе ресурсов.

   :::image type="content" source="../../../assets/images/adaptive-cards/go-to-resource-card.png" alt-text="На этом снимке экрана показано, как выбрать группу ресурсов.":::

    Теперь ваш бот Azure создан.

   :::image type="content" source="../../../assets/images/adaptive-cards/azure-bot-ui.png" alt-text="На этом снимке экрана показано, как создать ресурсы бота Azure.":::
...
1. В разделе **Параметры** выберите **Конфигурация**. Сохраните **Идентификатор приложения Майкрософт** (идентификатор клиента) для дальнейшего использования.
   ...
   :::image type="content" source="../../../assets/images/adaptive-cards/config-microsoft-app-id.png" alt-text="На этом снимке экрана показано, как добавить идентификатор приложения Майкрософт для создания секрета клиента.":::

1. Рядом с **идентификатором приложения Майкрософт** выберите " **Управление"**.

   :::image type="content" source="~/assets/images/manage-bot-label.png" alt-text="На этом снимке экрана показано, как создать бот для управления.":::

1. В разделе **Секреты клиента** выберите **Новый секрет клиента**. Появится окно **Добавить секрет клиента**.

   :::image type="content" source="../../../assets/images/meetings-side-panel/newclientsecret.PNG" alt-text="На этом снимке экрана показано, как создать секрет клиента.":::

1. Введите **описание** и нажмите **Добавить**.

   :::image type="content" source="../../../assets/images/adaptive-cards/client-secret.png" alt-text="На снимке экрана показано, как ввести описание секрета клиента.":::
...
1. В столбце **Значение** выберите **Копировать в буфер обмена** и сохраните идентификатор секрета клиента для дальнейшего использования.
   ...
   :::image type="content" source="../../../assets/images/adaptive-cards/client-secret-value.png" alt-text="На снимке экрана показано, как сохранить идентификатор секрета клиента для дальнейшей ссылки.":::

Чтобы добавить канал Microsoft Teams:

1. Перейдите на **домашнюю страницу**.

   :::image type="content" source="../../../assets/images/adaptive-cards/bot-home-page.png" alt-text="На этом снимке экрана показана домашняя страница бота.":::
...
1. Нажмите **Каналы** в левой области и выберите **Microsoft Teams** :::image type="icon" source="../../../assets/icons/teams-icon.png":::.
   ...
   :::image type="content" source="../../../assets/images/adaptive-cards/channel-teams.png" alt-text="На этом снимке экрана показано, как выбрать Teams в каналах.":::

1. Установите флажок, чтобы принять условия обслуживания, и выберите **Принять**.</br>

   :::image type="content" source="../../../assets/images/adaptive-cards/select-terms-of-service.png" alt-text="На этом снимке экрана показано, как задать условия использования службы.":::

1. Нажмите кнопку **Сохранить**.

   :::image type="content" source="../../../assets/images/adaptive-cards/select-teams.png" alt-text="На этом снимке экрана показано, как добавить канал Microsoft Teams.":::
...
1. Заполните форму следующим образом:
    ...
    1. **ИД клиента**. Введите **идентификатор каталога (клиент)**, записанный ранее для приложения удостоверений Azure, или **общий** вариант в зависимости от поддерживаемого типа учетной записи, выбранного при создании приложения поставщика удостоверений. Чтобы решить, какое значение следует назначить, выполните следующие условия:
        ...
        - Если вы выбрали учетные записи в любом каталоге организации (любой Microsoft Azure Active Directory (Azure AD) — несколько клиентов и личные учетные записи Майкрософт *, например Skype, Xbox, Outlook),* введите общее слово вместо идентификатора  клиента. В противном случае Azure AD (Azure AD) проверяется через клиент, идентификатор которого был выбран, и исключает личные учетные записи Майкрософт.
    ...
    :::image type="content" source="../../../assets/images/authentication/auth-bot-identity-connection-adv1.PNG" alt-text="На этом снимке экрана показано, как добавить подключение к удостоверению бота проверки подлинности бота Teams adv1.":::
...
1. Нажмите **Добавить параметры подключения OAuth**.  
   ...
   :::image type="content" source="../../../assets/images/authentication/sample-app-demo-bot-configuration.png" alt-text="На этом снимке экрана показан соответствующий выбор на странице ресурсов.":::

1. Заполните форму следующим образом:

    1. **Имя**. Введите имя подключения. Это имя будет использоваться в боте в файле `appsettings.json`. Например, *BotTeamsAuthADv2*.
    1. **Поставщик службы**. Выберите **Microsoft Azure Active Directory версии 2**. После выбора этого параметра отобразится Azure AD поля.
    1. **Идентификатор клиента**. Введите идентификатор приложения (клиент), записанный для приложения поставщика удостоверений Azure, на описанных выше шагах.
    1. **Секрет клиента**. Введите секрет, записанный для приложения поставщика удостоверений Azure, на описанных выше шагах.
    1. **URL-адрес Exchange маркера**. Не заполняйте это поле.
    1. **ИД клиента**. Введите **идентификатор каталога (клиент)**, записанный ранее для приложения удостоверений Azure, или **общий** вариант в зависимости от поддерживаемого типа учетной записи, выбранного при создании приложения поставщика удостоверений. Чтобы решить, какое значение следует назначить, выполните следующие условия:

        - Если вы выбрали учетные записи только в этом каталоге организации (только Microsoft — один клиент *)* или учетные записи в любом каталоге организации *(Microsoft Azure Active Directory — мультитенант)* введите идентификатор клиента, записанный ранее для приложения Azure AD. Это будет клиент, связанный с пользователями, которые могут проходить проверку подлинности.

        - Если вы выбрали учетные записи в любом каталоге организации (любой Microsoft Azure Active Directory (Azure AD) — несколько клиентов и личные учетные записи Майкрософт *, например Skype, Xbox, Outlook),* введите общее слово вместо идентификатора  клиента. В противном случае Azure AD проверяет клиент, идентификатор которого был выбран, и исключает личные учетные записи Майкрософт.
...
1. Затем вам будет предложено разрешить поставщику удостоверений использовать ваши данные (учетные данные). Указанное ниже изображение является примером.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-connection-test-accept.PNG" alt-text="На снимке экрана показано, как добавить строку подключения проверки подлинности бота Teams adv1.":::
...
1. После этого вы будете перенаправлены на страницу **Проверка подключения к \<your-connection-name> успешно пройдена**. Обновите страницу, если возникает ошибка. Указанное ниже изображение является примером.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-connection-test-token.PNG" alt-text="На снимке экрана показано, как добавить строку подключения проверки подлинности приложения Teams adv1.":::
...
1. В Обозреватель решений перейдите `TeamsAppManifest` в  папку, `manifest.json` `id` `botId` откройте и задайте идентификатор приложения бота, сохраненный во время регистрации бота.
...
1. В следующем диалоговом окне введите необходимые сведения. Ниже приведен пример.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-app-service.png" alt-text="На этом снимке экрана показано, как ввести необходимые сведения для службы приложений проверки подлинности.":::
...
1. В браузере перейдите [**к портал Azure**][azure-portal].
1. Проверьте группу ресурсов. Бот должен быть указан вместе с другими ресурсами. Указанное ниже изображение является примером.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-app-service-in-group.png" alt-text="На этом снимке экрана показано, как проверить группу ресурсов и бот.":::
...
1. В зависимости от конфигурации, используемой для Emulator, вы получите один из следующих результатов.
    ...
    :::image type="content" source="../../../assets/images/authentication/auth-bot-login-emulator.PNG" alt-text="На этом снимке экрана показан пример пользовательского интерфейса бота после входа в систему.":::

1. Если вы **выберете "** Да", когда бот спросит, хотите ли вы просмотреть маркер *?* Вы получите примерно такой ответ:

   :::image type="content" source="../../../assets/images/authentication/auth-bot-login-emulator-token.png" alt-text="На этом снимке экрана показано, как выбрать согласие.":::
...
1. При появлении запроса выберите учетную запись соответствующего пользователя.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-login-deployed.PNG" alt-text="На этом снимке экрана показан пример пользовательского интерфейса бота Teams после входа в систему.":::

1. Нажмите кнопку **Да**, чтобы отобразить маркер проверки подлинности. Указанное ниже изображение является примером.

   :::image type="content" source="../../../assets/images/authentication/auth-bot-login-deployed-token.PNG" alt-text="На этом снимке экрана показано, как нажатие кнопки &quot;Да&quot; для отображения маркера проверки подлинности.":::

1. Введите "выход", чтобы выйти.

   :::image type="content" source="../../../assets/images/authentication/auth-bot-deployed-logout.PNG" alt-text="На этом снимке экрана показано, как ввести выход для выхода.":::
...
1. В Обозреватель решений перейдите в папку`TeamsAppManifest`. Измените `manifest.json`, назначив следующие значения.
...
1. Перейдите в папку `TeamsAppManifest` и отправьте zip-манифест.
Отобразится следующий мастер.

   :::image type="content" source="../../../assets/images/authentication/auth-bot-teams-upload.png" alt-text="На этом снимке экрана показан пример бота после его отправки в Teams.":::
...
При этом запускается ngrok для прослушивания порта, который вы указали. В ответ он предоставляет доступный из внешней среды URL-адрес, действительный до тех пор, пока запущен ngrok. Указанное ниже изображение является примером.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-ngrok-start.PNG" alt-text="На этом снимке экрана показана строка подключения проверки подлинности приложения-бота Teams adv1":::

1. Скопируйте HTTPS-адрес переадресации. Он должен выглядеть примерно так: `https://dea822bf.ngrok.io/`.
1. Добавьте `/api/messages` для получения `https://dea822bf.ngrok.io/api/messages`. Это **конечная точка** сообщений для бота, запущенного локально на компьютере и доступного через Интернет в чате в Teams.
...
1. В окне терминала, где запущен `ngrok`, вы можете увидеть HTTP-трафик между ботом и клиентом веб-чата. Если требуется более подробное представление, в окне браузера введите значение `http://127.0.0.1:4040`, полученное из предыдущего окна терминала. Указанное ниже изображение является примером.
   ...
   :::image type="content" source="../../../assets/images/authentication/auth-bot-teams-ngrok-testing.png" alt-text="На этом снимке экрана показано тестирование ngrok группами ботов проверки подлинности.":::
