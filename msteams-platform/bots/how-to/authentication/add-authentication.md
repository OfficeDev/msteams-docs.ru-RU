...
1. <span data-ttu-id="4f0e5-147">В верхней левой части отображаемого окна выберите вкладку **Добавить,** чтобы создать новую группу ресурсов.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-147">In the upper left of the displayed window, select **Add** tab to create a new resource group.</span></span> <span data-ttu-id="4f0e5-148">Вам будет предложено предоставить следующее:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-148">You'll be prompted to provide the following:</span></span>
    ...
    1. <span data-ttu-id="4f0e5-158">Выберите **кнопку Создать.**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-158">Select the **Create** button.</span></span> <span data-ttu-id="4f0e5-159">Создание группы ресурсов может занять несколько минут.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-159">It may take a few minutes to create the resource group.</span></span>
...
1. <span data-ttu-id="4f0e5-167">Вам будет предложено предоставить следующие сведения:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-167">You'll be asked to provide the following information:</span></span>
    ...
    1. <span data-ttu-id="4f0e5-183">Выберите **кнопку Обзор и создайте** кнопку.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-183">Select the **Review and create** button.</span></span> <span data-ttu-id="4f0e5-184">Вы должны увидеть баннер, который считывал *пройденную проверку.*</span><span class="sxs-lookup"><span data-stu-id="4f0e5-184">You should see a banner that reads *Validation passed*.</span></span>
...
<span data-ttu-id="4f0e5-197">Вам нужен поставщик удостоверений, который можно использовать для проверки подлинности.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-197">You need an identity provider that can be used for authentication.</span></span>
...
1. <span data-ttu-id="4f0e5-240">Заполните форму следующим образом:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-240">Complete the form as follows:</span></span>
    ...
    1. <span data-ttu-id="4f0e5-249">**Секрет клиента.**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-249">**Client secret**.</span></span> <span data-ttu-id="4f0e5-250">Введите секрет, записанный для приложения-поставщика удостоверений Azure в шагах выше.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-250">Enter the secret that you recorded for your Azure identity provider app in the steps above.</span></span>
...
1. <span data-ttu-id="4f0e5-317">В C# обновление **appsettings.jsв** следующим образом:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-317">In C# Update **appsettings.json** as follows:</span></span>
    ...
    <span data-ttu-id="4f0e5-322">В зависимости от символов в секрете бота может потребоваться XML-пароль.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-322">Depending on the characters in your bot secret, you may need to XML escape the password.</span></span> <span data-ttu-id="4f0e5-323">Например, любые амперанды (&) должны быть закодированы как `&amp;` .</span><span class="sxs-lookup"><span data-stu-id="4f0e5-323">For example, any ampersands (&) will need to be encoded as `&amp;`.</span></span>
...
# <a name="python"></a>[<span data-ttu-id="4f0e5-336">Python</span><span class="sxs-lookup"><span data-stu-id="4f0e5-336">Python</span></span>](#tab/python)
...
<span data-ttu-id="4f0e5-371">Чтобы образец бота работал, необходимо настроить эмулятор, как показано ниже.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-371">In order for the bot sample login to work you must configure the Emulator as shown below.</span></span>
...
<span data-ttu-id="4f0e5-386">После настройки механизма проверки подлинности можно выполнить фактическое тестирование бота.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-386">After you have configured the authentication mechanism, you can perform the actual bot testing.</span></span>  
...
1. <span data-ttu-id="4f0e5-401">В зависимости от конфигурации эмулятора вы получаете одну из следующих конфигураций:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-401">Depending which configuration you used for the Emulator, you get one of the following:</span></span>
    ...
    <span data-ttu-id="4f0e5-407">Ниже приводится пример пользовательского интерфейса бота после входа в систему:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-407">The following image is an example of the bot UI after you've logged in:</span></span>
...
1. <span data-ttu-id="4f0e5-426">При запросе выберите учетную запись соответствующего пользователя.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-426">If asked, select the applicable user's account.</span></span>
    ...
    ![Развернутый вход бота auth](../../../assets/images/authentication/auth-bot-login-deployed.PNG)<span data-ttu-id="4f0e5-429">.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-429">.</span></span>
...
<span data-ttu-id="4f0e5-460">Microsoft Teams является полностью облачным продуктом, он требует, чтобы все доступные ему службы были доступны в облаке с помощью конечных точек HTTPS.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-460">Microsoft Teams is an entirely cloud-based product, it requires all services it accesses to be available from the cloud using HTTPS endpoints.</span></span> <span data-ttu-id="4f0e5-461">Поэтому для того, чтобы бот (наш пример) работал в Teams, необходимо либо опубликовать код в облако по вашему выбору, либо  сделать локально запущенный экземпляр внешне доступным с помощью средства туннеля.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-461">Therefore, to enable the bot (our sample) to work in Teams, you need to either publish the code to the cloud of your choice, or make a locally running instance externally accessible via a **tunneling** tool.</span></span> <span data-ttu-id="4f0e5-462">Рекомендуется  [ngrok,](https://ngrok.com/download)который создает внешне адресируемый URL-адрес для порта, который вы открываете локально на компьютере.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-462">We recommend  [ngrok](https://ngrok.com/download), which creates an externally addressable URL for a port you open locally on your machine.</span></span>
<span data-ttu-id="4f0e5-463">Чтобы настроить ngrok в процессе подготовки к локальному Microsoft Teams приложения, выполните следующие действия:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-463">To set up ngrok in preparation for running your Microsoft Teams app locally, follow these steps:</span></span>

1. <span data-ttu-id="4f0e5-464">В окне терминала перейдите к каталогу, в котором вы `ngrok.exe` установили.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-464">In a terminal window, go the directory where you have `ngrok.exe` installed.</span></span> <span data-ttu-id="4f0e5-465">Мы предлагаем указать на *него* путь переменной среды.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-465">We suggest setting the *environment variable* path to point to it.</span></span>
1. <span data-ttu-id="4f0e5-466">Запустите, например, `ngrok http 3978 --host-header=localhost:3978` .</span><span class="sxs-lookup"><span data-stu-id="4f0e5-466">Run, for example, `ngrok http 3978 --host-header=localhost:3978`.</span></span> <span data-ttu-id="4f0e5-467">Замените номер порта по мере необходимости.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-467">Replace the port number as needed.</span></span>
<span data-ttu-id="4f0e5-468">В этом случае запускается ngrok для прослушивания в порту, который вы указываете.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-468">This launches ngrok to listen on the port you specify.</span></span> <span data-ttu-id="4f0e5-469">Взамен он предоставляет внешне адресируемый URL-адрес, допустимый до тех пор, пока запущен ngrok.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-469">In return, it gives you an externally addressable URL, valid for as long as ngrok is running.</span></span> <span data-ttu-id="4f0e5-470">В качестве примера приводится следующее изображение:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-470">The following image is an example:</span></span>

    ![teams bot app auth connection string adv1](../../../assets/images/authentication/auth-bot-ngrok-start.PNG)<span data-ttu-id="4f0e5-472">.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-472">.</span></span>

1. <span data-ttu-id="4f0e5-473">Скопируйте адрес HTTPS-переададки.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-473">Copy the forwarding HTTPS address.</span></span> <span data-ttu-id="4f0e5-474">Он должен быть похож на следующее: `https://dea822bf.ngrok.io/` .</span><span class="sxs-lookup"><span data-stu-id="4f0e5-474">It should be similar to the following: `https://dea822bf.ngrok.io/`.</span></span>
1. <span data-ttu-id="4f0e5-475">Приложение `/api/messages` для получения `https://dea822bf.ngrok.io/api/messages` .</span><span class="sxs-lookup"><span data-stu-id="4f0e5-475">Append `/api/messages` to obtain `https://dea822bf.ngrok.io/api/messages`.</span></span> <span data-ttu-id="4f0e5-476">Это **конечная** точка сообщений для бота, запущенного локально на вашем компьютере и достигаемого в Интернете в чате в Microsoft Teams.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-476">This is the **messages endpoint** for the bot running locally on your machine and reachable over the web in a chat in Microsoft Teams.</span></span>
1. <span data-ttu-id="4f0e5-477">Последний шаг для выполнения — обновление конечной точки сообщений развернутого бота.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-477">One final step to perform is to update the messages endpoint of the deployed bot.</span></span> <span data-ttu-id="4f0e5-478">В примере мы развернули бот в Azure.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-478">In the example, we deployed the bot in Azure.</span></span> <span data-ttu-id="4f0e5-479">Итак, \*\*давайте выполните следующие действия:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-479">So \*\*let's perform these steps:</span></span>
    1. <span data-ttu-id="4f0e5-480">В браузере перейдите на портал [**Azure.**][azure-portal]</span><span class="sxs-lookup"><span data-stu-id="4f0e5-480">In your browser navigate to the [**Azure portal**][azure-portal].</span></span>
    1. <span data-ttu-id="4f0e5-481">Выберите **регистрацию бот-канала.**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-481">Select your **Bot Channel Registration**.</span></span>
    1. <span data-ttu-id="4f0e5-482">В левой панели выберите **Параметры**.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-482">In the left panel, select **Settings**.</span></span>
    1. <span data-ttu-id="4f0e5-483">В правой панели в конечную точку **обмена** сообщениями введите URL-адрес ngrok в нашем `https://dea822bf.ngrok.io/api/messages` примере.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-483">In the right panel, in the **Messaging endpoint** box, enter the ngrok URL, in our example, `https://dea822bf.ngrok.io/api/messages`.</span></span>
1. <span data-ttu-id="4f0e5-484">Запустите бот локально, например в режиме Visual Studio отлаживания.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-484">Start your bot locally, for example in Visual Studio debug mode.</span></span>
1. <span data-ttu-id="4f0e5-485">Тестирование бота при локальном запуске с помощью тестового веб-чата портала Bot **Framework.**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-485">Test the bot while running locally using the Bot Framework portal's **Test Web chat**.</span></span> <span data-ttu-id="4f0e5-486">Как и эмулятор, этот тест не позволяет получить доступ к Teams определенным функциям.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-486">Like the Emulator, this test doesn't allow you to access Teams-specific functionality.</span></span>
1. <span data-ttu-id="4f0e5-487">В окне терминала, где работает, можно увидеть трафик HTTP между `ngrok` ботом и клиентом веб-чата.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-487">In the terminal window where `ngrok` is running you can see HTTP traffic between the bot and the web chat client.</span></span> <span data-ttu-id="4f0e5-488">Если вам нужно более подробное представление, в окно браузера введите `http://127.0.0.1:4040` вас, полученную из предыдущего окна терминала.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-488">If you want a more detailed view, in a browser window enter `http://127.0.0.1:4040` you obtained from the previous terminal window.</span></span> <span data-ttu-id="4f0e5-489">В качестве примера приводится следующее изображение:</span><span class="sxs-lookup"><span data-stu-id="4f0e5-489">The following image is an example:</span></span>

    ![Auth bot teams ngrok testing](../../../assets/images/authentication/auth-bot-teams-ngrok-testing.png)<span data-ttu-id="4f0e5-491">.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-491">.</span></span>

> [!NOTE]
> <span data-ttu-id="4f0e5-492">Если остановить и перезапустить ngrok, URL-адрес изменится.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-492">If you stop and restart ngrok, the URL changes.</span></span> <span data-ttu-id="4f0e5-493">Чтобы использовать ngrok в проекте и в зависимости от возможностей, которые вы используете, необходимо обновить все ссылки на URL-адрес.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-493">To use ngrok in your project, and depending on the capabilities you're using, you must update all URL references.</span></span>
 

## <a name="additional-information"></a><span data-ttu-id="4f0e5-494">Дополнительные сведения</span><span class="sxs-lookup"><span data-stu-id="4f0e5-494">Additional information</span></span>

### <a name="teamsappmanifestmanifestjson"></a><span data-ttu-id="4f0e5-495">TeamsAppManifest/manifest.js</span><span class="sxs-lookup"><span data-stu-id="4f0e5-495">TeamsAppManifest/manifest.json</span></span>

<span data-ttu-id="4f0e5-496">Этот манифест содержит сведения, необходимые Microsoft Teams для подключения к боту.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-496">This manifest contains information needed by Microsoft Teams to connect with the bot.</span></span>  
...
<span data-ttu-id="4f0e5-497">При проверке подлинности Teams ведет себя немного иначе, чем по другим каналам, как поясняется ниже.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-497">With authentication, Teams behaves slightly differently than other channels, as explained below.</span></span>

### <a name="handling-invoke-activity"></a><span data-ttu-id="4f0e5-498">Обработка действия вызовов</span><span class="sxs-lookup"><span data-stu-id="4f0e5-498">Handling Invoke Activity</span></span>

<span data-ttu-id="4f0e5-499">Действие **invoke отправляется** боту, а не действию событий, используемой другими каналами.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-499">An **Invoke Activity** is sent to the bot rather than the Event Activity used by other channels.</span></span>
<span data-ttu-id="4f0e5-500">Это делается путем подклассинга **ActivityHandler**.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-500">This is done by sub-classing the **ActivityHandler**.</span></span>

# <a name="cnet"></a>[<span data-ttu-id="4f0e5-501">C#/.NET</span><span class="sxs-lookup"><span data-stu-id="4f0e5-501">C#/.NET</span></span>](#tab/dotnet-sample)

<span data-ttu-id="4f0e5-502">**Bots/DialogBot.cs**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-502">**Bots/DialogBot.cs**</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/DialogBot.cs?range=19-51)]

<span data-ttu-id="4f0e5-503">**Bots/TeamsBot.cs**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-503">**Bots/TeamsBot.cs**</span></span>

<span data-ttu-id="4f0e5-504">Если *используется* **OAuthPrompt,** в диалоговое окно необходимо перенаправлить действие Invoke Activity.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-504">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/TeamsBot.cs?range=34-42)]

#### <a name="teamsactivityhandlercs"></a><span data-ttu-id="4f0e5-505">TeamsActivityHandler.cs</span><span class="sxs-lookup"><span data-stu-id="4f0e5-505">TeamsActivityHandler.cs</span></span>
...
# <a name="javascript"></a>[<span data-ttu-id="4f0e5-506">JavaScript</span><span class="sxs-lookup"><span data-stu-id="4f0e5-506">JavaScript</span></span>](#tab/node-js-dialog-sample)

<span data-ttu-id="4f0e5-507">**bots/dialogBot.js**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-507">**bots/dialogBot.js**</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/dialogBot.js?range=4-46)]

<span data-ttu-id="4f0e5-508">**bots/teamsBot.js**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-508">**bots/teamsBot.js**</span></span>

<span data-ttu-id="4f0e5-509">Если *используется* **OAuthPrompt,** в диалоговое окно необходимо перенаправлить действие Invoke Activity.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-509">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/teamsBot.js?range=4-33)]

<span data-ttu-id="4f0e5-510">**диалоги/mainDialog.js**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-510">**dialogs/mainDialog.js**</span></span>

<span data-ttu-id="4f0e5-511">В диалоговом шаге используйте `beginDialog` для запуска запроса OAuth, в котором пользователь должен войти.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-511">Within a dialog step, use `beginDialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="4f0e5-512">Если пользователь уже подписан, это создает событие отклика маркеров без запроса пользователя.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-512">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="4f0e5-513">В противном случае пользователь должен войти.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-513">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="4f0e5-514">Служба ботов Azure отправляет событие отклика маркера после попытки пользователя войти.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-514">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-52)]

<span data-ttu-id="4f0e5-515">В следующем диалоговом шаге проверьте наличие маркера в результате предыдущего шага.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-515">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="4f0e5-516">Если это не null, пользователь успешно вписался.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-516">If it is not null, the user successfully signed in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-64)]

<span data-ttu-id="4f0e5-517">**bots/logoutDialog.js**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-517">**bots/logoutDialog.js**</span></span>

[!code-javascript[allow-logout](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/logoutDialog.js?range=31-42&highlight=7)]

# <a name="python"></a>[<span data-ttu-id="4f0e5-518">Python</span><span class="sxs-lookup"><span data-stu-id="4f0e5-518">Python</span></span>](#tab/python-sample)

<span data-ttu-id="4f0e5-519">**bots/dialog_bot.py**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-519">**bots/dialog_bot.py**</span></span>

[!code-python[ActivityHandler](~/../botbuilder-samples/samples/python/46.teams-auth/bots/dialog_bot.py?range=10-42)]

<span data-ttu-id="4f0e5-520">**bots/teams_bot.py**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-520">**bots/teams_bot.py**</span></span>

<span data-ttu-id="4f0e5-521">Если *используется* **OAuthPrompt,** в диалоговое окно необходимо перенаправлить действие Invoke Activity.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-521">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-python[on_token_response_event](~/../botbuilder-samples/samples/python/46.teams-auth/bots/teams_bot.py?range=38-45)]

<span data-ttu-id="4f0e5-522">**диалоги/main_dialog.py**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-522">**dialogs/main_dialog.py**</span></span>

<span data-ttu-id="4f0e5-523">В диалоговом шаге используйте `begin_dialog` для запуска запроса OAuth, в котором пользователь должен войти.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-523">Within a dialog step, use `begin_dialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="4f0e5-524">Если пользователь уже подписан, это создает событие отклика маркеров без запроса пользователя.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-524">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="4f0e5-525">В противном случае пользователь должен войти.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-525">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="4f0e5-526">Служба ботов Azure отправляет событие отклика маркера после попытки пользователя войти.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-526">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=48-49)]

<span data-ttu-id="4f0e5-527">В следующем диалоговом шаге проверьте наличие маркера в результате предыдущего шага.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-527">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="4f0e5-528">Если это не null, пользователь успешно вписался.</span><span class="sxs-lookup"><span data-stu-id="4f0e5-528">If it is not null, the user successfully signed in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=51-61)]

<span data-ttu-id="4f0e5-529">**диалоги/logout_dialog.py**</span><span class="sxs-lookup"><span data-stu-id="4f0e5-529">**dialogs/logout_dialog.py**</span></span>
...
> [<span data-ttu-id="4f0e5-530">Узнайте о добавлении проверки подлинности с помощью службы Azure Bot</span><span class="sxs-lookup"><span data-stu-id="4f0e5-530">Learn about adding authentication via Azure Bot Service</span></span>](https://aka.ms/azure-bot-add-authentication)
