...
Note: для поставщиков услуг здесь доступны два варианта: Azure AD v1 и Azure AD v2.  Различия между двумя поставщиками обобщены [здесь](https://docs.microsoft.com/azure/active-directory/azuread-dev/azure-ad-endpoint-comparison), но в общем случае v2 предоставляет больше гибкости относительно изменения разрешений Bot.  Разрешения API Graph перечислены в поле области, и при добавлении новых разрешений Боты позволит пользователям подтвердить новые разрешения при следующем входе в систему.  Для v1 пользователь должен удалить согласие с приглашением на добавление новых разрешений в диалоговом окне OAuth. 
...
1. Заполните форму следующим образом:
    ...
    1. **Name (имя** ). Введите имя подключения. Это имя будет использоваться в `appsettings.json` файле Bot. Например, *BotTeamsAuthADv2*.
    1. **Поставщик услуг**. Выберите **Azure Active Directory v2**. После выбора этого поля будут отображены поля, связанные с Azure AD.
    1. **Идентификатор клиента**. Введите идентификатор приложения (клиента), записанный для приложения поставщика удостоверений Azure, в описанных выше шагах.
    1. **Секрет клиента**. Введите секретный код, записанный для приложения поставщика удостоверений Azure, в описанных выше шагах.
    1. **URL-адрес для обмена маркерами**. Оставьте это поле пустым.
    1. **Идентификатор клиента** введите **идентификатор каталога (клиента)** , записанный ранее для своего приложения-удостоверения Azure, или **Общий** , в зависимости от поддерживаемого типа учетной записи, выбранной при создании приложения поставщика удостоверений. Чтобы выбрать значение, которое нужно назначить, выполните следующие условия:
...
1. В C# обновление **appsettings.js** следующим образом:
...
Будет запущен ngrok для прослушивания указанного порта. В качестве возврата вы получите URL-адрес с внешним адресом, допустимый до тех пор, пока работает ngrok. Ниже приведен пример такого изображения.
    ...
    ![Строка подключения к приложению подлинности подлинности для adv1 Teams](../../../assets/images/authentication/auth-bot-ngrok-start.PNG).
...
### <a name="teamsappmanifestmanifestjson"></a>Теамсаппманифест/manifest.jsна
...
**диалоговые окна и mainDialog.js**
...
> [Сведения о добавлении проверки подлинности через службу Azure Bot](https://aka.ms/azure-bot-add-authentication)
