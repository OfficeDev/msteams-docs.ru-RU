...
1. Вам будет предложено указать следующие сведения:
    ...
    1. Нажмите кнопку " **Просмотр и создание** ". Вы должны увидеть баннер с прочтением *пройденной проверки*.
    1. Нажмите **Создать**. Создание плана служб приложений может занять несколько минут. План будет указан в группе ресурсов.
...
1. В `teamsAppManifest` `manifest.json` папке откройте и укажите `id` **идентификатор приложения Microsoft** и `botId` **идентификатор приложения** , сохраненного во время регистрации канала ленты.
...
# <a name="cnet"></a>[ЯЗЫК C#/.НЕТ](#tab/dotnet-sample)
