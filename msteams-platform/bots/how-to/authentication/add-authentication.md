...
- **Создание ленты с включенной проверкой подлинности**. Вы будете использовать [CS – auth – Sample][teams-auth-bot] для обработки учетных данных пользователя и создания маркера проверки подлинности.
...
- Visual Studio 2017 или более поздней версии, а также Git.
...
- Приведенный ниже пример.
    ...
    |:---|:---:|:---|
    | **Проверка подлинности с помощью подлинности** в [CS – auth — пример][teams-auth-bot] | IPv4 | Поддержка Оаускард |
    | Простая **Проверка подлинности** в [Python – auth — пример][teams-auth-bot-py] | IPv4 | Поддержка Оаускард |
...
1. Заполните форму следующим образом:
    ...
    высоты. Введите в поле `https://graph.microsoft.com/` **URL-адрес ресурса**. Этот параметр не используется в текущем примере кода.  
    узнать. Оставьте **области** пустыми. Ниже приведен пример такого изображения.
...
1. После этого необходимо перенаправить вас на **пробное \<подключение к странице "-connection-Name> SUCCEEDED** . Обновите страницу при возникновении ошибки. Ниже приведен пример такого изображения.
...
1. Clone [CS — auth — пример][teams-auth-bot].
...
1. В C# обновите **appSettings. JSON** следующим образом:
    ...
    - Задайте для `ConnectionName` параметра имя подключения поставщика удостоверений. 
...
1. В обозревателе решений перейдите `TeamsAppManifest` к папке, откройте `manifest.json` и установите `id` `botId` **идентификатор приложения** , сохраненного во время регистрации канала ленты.

# <a name="pythontabpython"></a>[Python](#tab/python)

1. Клонировать пример [проверки подлинности][teams-auth-bot-py] с помощью кода Bot в репозитории GitHub.
1. Обновление **config.py**:

    - Укажите `ConnectionName` имя параметра подключения OAuth, добавленного к почтовому роботу.
    - Задайте `MicrosoftAppId` идентификатор `MicrosoftAppPassword` приложения и секрет приложения для пользователя Bot.
...
1. В поле **Конечная точка для обмена сообщениями** введите URL-адрес `api/messages`, полученный выше. Пример: `https://botteamsauth.azurewebsites.net/api/messages`.
...
1. В проекте `TeamsAppManifest` Bot убедитесь, что папка содержит `manifest.json` вместе с файлами `outline.png` и. `color.png`
...
1. Выбор и **почтовые индексы** для `manifest.json` `outline.png`и `color.png` файлов.
...
- [Добавление проверки подлинности в Bot через службу Bot Azure](https://aka.ms/azure-bot-add-authentication)
