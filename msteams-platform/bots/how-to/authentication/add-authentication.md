...
- **Как развернуть бот в Azure и связать его с поставщиком удостоверений**. Поставщик выпускает маркер на основе учетных данных для входа пользователя. Бот может использовать маркер для доступа к ресурсам, таким как почтовая служба, для которых требуется проверка подлинности. Дополнительные сведения см. в статье [Поток проверки подлинности для ботов в Microsoft Teams](auth-flow-bot.md).
...
Регистрация ресурсов Azure Bot регистрирует вашу веб-службу в качестве бота в Bot Framework, что предоставляет вам идентификатор приложения Майкрософт и пароль приложения (секрет клиента).
...
В этой процедуре вы будете использовать поставщика Azure AD. Также можно использовать других поставщиков удостоверений, поддерживаемых Azure AD.
...
1. Вам будет предложено указать следующую информацию.
   ...
   1. Выберите **поддерживаемые типы учетных записей** для приложения. Выберите *Учетные записи в любом каталоге организации (любой каталог Azure AD — мультитенантный) и персональные учетные записи Майкрософт (например, Skype, Xbox)*.
...
1. Когда приложение будет создано, Azure отобразит страницу **Обзор** для него. Скопируйте и сохраните следующие сведения в файл.
...
1. Заполните форму следующим образом:
    ...
    1. **ИД клиента**. Введите **идентификатор каталога (клиент)**, записанный ранее для приложения удостоверений Azure, или **общий** вариант в зависимости от поддерживаемого типа учетной записи, выбранного при создании приложения поставщика удостоверений. Чтобы решить, какое значение следует назначить, используйте следующие условия.

        - Если вы выбрали *Учетные записи только в этом каталоге организации (только RSS — один клиент)* или *Учетные записи в любом каталоге организации (любой каталог Azure AD — мультитенантный)* введите **идентификатор клиента**, записанный ранее для приложения Microsoft Azure Active Directory (Azure AD). Это будет клиент, связанный с пользователями, которые могут проходить проверку подлинности.

        - Если вы выбрали *Учетные записи в любом каталоге организации (любой каталог Azure AD — мультитенантный) и персональные учетные записи Майкрософт (например, Skype, Xbox, Outlook)*, введите слово **общий** вместо идентификатора клиента. В противном случае приложение Microsoft Azure Active Directory (Azure AD) выполнит проверку через клиент, идентификатор которого был выбран, и исключит личные учетные записи Майкрософт.
...
1. Заполните форму следующим образом:
    ...
    1. **ИД клиента**. Введите **идентификатор каталога (клиент)**, записанный ранее для приложения удостоверений Azure, или **общий** вариант в зависимости от поддерживаемого типа учетной записи, выбранного при создании приложения поставщика удостоверений. Чтобы решить, какое значение следует назначить, используйте следующие условия.
        ...
        - Если вы выбрали *Учетные записи только в этом каталоге организации (только RSS — один клиент)* или *Учетные записи в любом каталоге организации (любой каталог Microsoft Azure Active — мультитенантный)* введите **идентификатор клиента**, записанный ранее для приложения Microsoft Azure Active Directory (Azure AD). Это будет клиент, связанный с пользователями, которые могут проходить проверку подлинности.

        - Если вы выбрали *Учетные записи в любом каталоге организации (любой каталог Azure AD — мультитенантный) и персональные учетные записи Майкрософт (например, Skype, Xbox, Outlook)*, введите слово **общий** вместо идентификатора клиента. В противном случае приложение Microsoft Azure Active Directory (Azure AD) выполнит проверку через клиент, идентификатор которого был выбран, и исключит личные учетные записи Майкрософт.

    1. В разделе **Области** введите разделенный пробелами список разрешений Graph, необходимых этому приложению, например User.Read User.ReadBasic.All Mail.Read
...
1. На панели инструментов выберите **Файл -> Открыть -> Проект или решение** и откройте проект бота.
1. В C# обновите **appsettings.json** указанным ниже образом.
...
Чтобы развернуть бот, выполните инструкции по [развертыванию бота в Azure](https://aka.ms/azure-bot-deployment-cli).

Кроме того, в Visual Studio вы можете выполнить следующие действия.

1. В *обозревателе решений* Visual Studio нажмите и удерживайте (или щелкните правой кнопкой мыши) имя проекта.
...
После настройки механизма проверки подлинности вы можете выполнить фактическое тестирование бота.  
