...
Чтобы защитить Microsoft Teams и его пользователей, API бота предоставляют ограничение скорости для входящих запросов. Приложения, которые преодолеют это ограничение, получают состояние `HTTP 429 Too Many Requests` ошибки. Все запросы подчиняются одной и той же политике ограничения скорости, включая отправку сообщений, список каналов и извлечение реестра.

Поскольку точные значения ограничений скорости могут изменяться, ваше приложение должно реализовать соответствующее поведение при возврате API `HTTP 429 Too Many Requests`.

## <a name="handle-rate-limits"></a>Ограничения скорости обработки

При выдаче SDK-операции Bot Builder можно `Microsoft.Rest.HttpOperationException` обрабатывать и проверять код состояния.
...
После обработки ограничений скорости для ботов можно `HTTP 429` обрабатывать ответы с помощью экспоненциального отработки.
...
После обработки `HTTP 429` ответов можно перейти к примеру для обнаружения временных исключений.

> [!NOTE]
> Помимо повторной работы кода **ошибки 429**, необходимо также повторно использовать коды **ошибок 412**, **502** и **504** .
...
Вы можете выполнять отработку и повторное выполнение с помощью [преходящей обработки сбоя](/previous-versions/msp-n-p/hh675232%28v%3dpandp.10%29). Рекомендации по получению и установке пакета NuGet см. в добавлении к решению блока обработки [сбоя](/previous-versions/msp-n-p/dn440719(v=pandp.60)?redirectedfrom=MSDN). См. [также переходную обработку с ошибками](/azure/architecture/best-practices/transient-faults).
...
Дополнительные сведения см. [в шаблонах повторного получения сведений](/azure/architecture/patterns/retry).
...
>[!NOTE]
> Предыдущие версии и `TeamsInfo.getMembers` API `TeamsInfo.GetMembersAsync` отстают. Они отлаговываются до пяти запросов в минуту и возвращают не более 10 000 участников на каждую команду. Чтобы обновить SDK bot Framework и код для использования последних конечных точек API paginated, см. изменения API bot для членов [группы и чата](../../resources/team-chat-member-api-changes.md).
