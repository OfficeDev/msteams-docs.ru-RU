...
ms.locfileid: "65111810"
---
# <a name="optimize-your-bot-with-rate-limiting-in-teams"></a>Оптимизация бота с ограничением скорости в Teams

Ограничение скорости — это метод ограничения максимальной частоты сообщений. Как правило, приложение должно ограничить количество сообщений, которые оно публикует, в отдельном чате или беседе канала. Это гарантирует оптимальное взаимодействие, а сообщения не кажутся пользователям спамом.
...
Для обработки 429-х рекомендуется использовать экспоненциальный отход с произвольным дрожанием. Это гарантирует, что при нескольких запросах не будут происходить конфликты при повторном запросе.
...
Ограничение на бота на поток управляет трафиком, который бот может создавать в одной беседе. Беседа — это личный диалог между ботом и пользователем, групповым чатом или каналом в команде. Таким образом, если приложение отправляет каждому пользователю по одному сообщению бота, ограничение потока не регулируется.
...
Ограничение на поток для всех ботов управляет трафиком, который все боты могут создавать в одной беседе. Здесь беседа — это личный диалог между ботом и пользователем, групповым чатом или каналом в команде.
