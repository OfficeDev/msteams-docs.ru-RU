...
Боты доступны в чатах, каналах и личных приложениях. Бота можно добавить одним из указанных ниже способов.
...
В личных контекстах приветствия задают тон бота. Сообщение включает в себя приветствие, что бот может сделать, и некоторые предложения о том, как взаимодействовать. Например, "Попробуй спросить меня о ...". По возможности эти предложения должны возвращать готовые ответы, чтобы пользователю не обязательно было входить в систему.
...
### <a name="introductions-in-group-chats-and-channels"></a>Представление бота в групповых чатах и каналах

Представление бота в групповых чатах и каналах должно немного отличаться от представления в личном контексте (например, в личном приложении). В реальной жизни, войдя в комнату, заполненную людьми, вы бы не стали приветствовать тех, кто там уже находится; вместо этого вы бы представились. То же самое надо воплотить в конструкции бота.
...
Вы можете включить обзорное видео с приветствиями и указать, отвечает ли бот на что-нибудь вроде команды "справка". Обзор — самый эффективный способ описать, что может сделать бот. Если это применимо, они также отлично под названием описывают другие функции вашего приложения. Например, включите скриншоты расширения обмена сообщениями.
...
В личном приложении карусель может предоставить эффективный обзор бота и любых других функций приложения. Включая кнопки, которые позволяют пользователям пробовать команды ботов, рекомендуется. Например, **Создайте задачу.**
...
В следующих примерах излагаются намерения пользователей и данные в сообщениях, отправленных ботам:
...
В следующем примере бот отвечает на каждое сообщение вариантами того, что может захотеть сделать дальше:
...
В следующем примере пользователь получает уведомление о всплывающих уведомлениях о том, что бот отправил им сообщение в другом канале:
...
Вкладка упрощает работу с ботом. Например, если ваш бот может создавать рабочие элементы, было бы неплохо показать все эти элементы в центральном месте внутри вкладки. Для получения дополнительной информации [можно посмотреть вкладки проектирования.](../../tabs/design/tabs.md)
