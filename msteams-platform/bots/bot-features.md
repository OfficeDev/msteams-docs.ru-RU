...
Каналы содержат цепочки разговоров между несколькими людьми, даже до двух тысяч. Это потенциально дает вашему боту огромный охват, но отдельные взаимодействия должны быть краткими. Традиционные множественные взаимодействия не работают. Вместо этого лучше использовать интерактивные карточки или модули задач, или переместиться в индивидуальную беседу, чтобы собрать больше сведений. Ваш бот имеет доступ только к сообщениям, в которых он `@mentioned`. Вы можете получить дополнительные сообщения из беседы с помощью Microsoft Graph и разрешений на уровне организации.
...
Групповые чаты — это не потоковые беседы между тремя или более пользователями. Как правило, у них меньше участников, чем у канала, и они более временные. Как и в канале, ваш бот имеет доступ только к тем сообщениям, где он непосредственно `@mentioned`.
...
Подробный диалог между вашим ботом и пользователем — это медленный и сложный способ выполнения задачи. Бот, который поддерживает слишком много команд, особенно широкий спектр команд, не имеет успеха и не находит положительного отклика у пользователей.

### <a name="have-multi-turn-experiences-in-chat"></a>Общение в чате с несколькими подходами

Для активного диалога разработчику требуется поддерживать состояние. Чтобы выйти из этого состояния, пользователь должен либо выдержать тайм-аут, либо нажать кнопку **Отмена**. К тому же процесс утомительный. Рассмотрим следующий сценарий разговора:
...
Одним из недостатков ботов является то, что сложно поддерживать большую базу знаний поиска с рядовыми ответами. Боты лучше всего подходят для коротких и быстрых взаимодействий, а не для просеивания длинных списков в поисках ответа.
