...
* `bots[0].supportsCalling`. Если вы присутствуете `true`и Teams позволяет вашему боту участвовать в звонках и собраниях в Интернете.
* `bots[0].supportsVideo`. Если вы присутствуете `true`и Teams, что ваш бот поддерживает видео.

Если вы хотите, чтобы ваш IDE правильно проверял схему manifest.json для бота вызовов и собраний для этих значений, `$schema` вы можете изменить атрибут следующим образом:
...
1. Используйте эту ссылку для создания нового бота. `https://dev.botframework.com/bots/new` Кроме того, если вы выберите кнопку **Создание** бота на портале Bot Framework, вы создайте бот в Microsoft Azure, для которого должна быть учетная запись Azure.
...
1. Выберите **вкладку Вызов** на Teams канале. Выберите **Включить вызов**, а затем обновить **веб-ок (** для вызова) с URL-адресом HTTPS, где вы получаете входящие уведомления, например `https://contoso.com/teamsapp/api/calling`. Дополнительные сведения см. [в настройках каналов](/bot-framework/portal-configure-channels).
    ...
    ![Настройка Teams каналов](~/assets/images/calls-and-meetings/configure-msteams-channel.png)
...
В Graph предоставляется детализация разрешений для управления доступом приложений к ресурсам. Вы решаете, какие разрешения Graph приложения. В Graph API поддерживаются разрешения приложений, которые используются приложениями, которые работают без участия пользователя. Администратор клиента должен предоставить согласие на разрешения приложений.
...
Чтобы использовать конечную точку [Microsoft Azure Active Directory Azure AD](/azure/active-directory/develop/azure-ad-endpoint-comparison), необходимо заранее настроить разрешения [](https://aka.ms/aadapplist) приложения для бота с помощью Microsoft Azure портала.

### <a name="get-tenant-administrator-consent"></a>Получить согласие администратора клиента

Для приложений, использующих конечную точку Azure AD V1, администратор клиента может дать согласие на разрешения приложений с помощью портала Microsoft Azure, когда [ваше](https://portal.azure.com) приложение установлено в организации. Кроме того, вы можете предоставить в приложении возможность регистрации, с помощью которой администраторы могут дать согласие на настроенные разрешения. После записи согласия администратора в Azure AD приложение может запрашивать маркеры без повторного запроса согласия.

Вы можете использовать администратора для предоставления разрешений, необходимых вашему приложению на Microsoft Azure [портале](https://portal.azure.com). Лучшим вариантом является предоставление администраторам возможности регистрации с помощью конечной точки Azure AD V2 `/adminconsent` . Дополнительные сведения см. [в инструкции по построению URL-адреса согласия администратора](/graph/auth-v2-service#3-get-administrator-consent).

> [!NOTE]
> Чтобы создать URL-адрес согласия администратора клиента, необходимо настроить URI перенаправления или url-адрес ответа на портале регистрации приложений.[](https://apps.dev.microsoft.com/) Чтобы добавить URL-адреса ответов для бота, выберите манифест **приложения Advanced** **OptionsEdit** > . Добавьте URL-адрес перенаправления в коллекцию `replyUrls` .
...
* [Просмотр разрешения приложения и согласия администратора на предоставление](/MicrosoftTeams/app-permissions-admin-center)
