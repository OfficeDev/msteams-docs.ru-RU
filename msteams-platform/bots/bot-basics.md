...
author: surbhigupta
description: Сведения о событиях и обработчиках действий Microsoft Teams для сообщений, каналов, команд, участников, упоминаний, проверки подлинности, действий с карточками с помощью пакета SDK для Microsoft Bot Framework.
ms.topic: conceptual
...
Чтобы реализовать логику для конкретных обработчиков действий Teams, необходимо переопределить методы в боте, как показано в разделе [Логика бота](#bot-logic). Для этих обработчиков нет базовой реализации. Поэтому добавьте в переопределение нужную логику.
...
| actionableMessage/executeAction | `OnTeamsO365ConnectorCardActionAsync` | Этот метод вызывается при получении от соединителя действия Office 365 карточки соединителя. |
...
Список обработчиков действий Teams, вызываемых из `onInvokeActivity` обработчика действий Teams, включает следующие:
...
| actionableMessage/executeAction | `handleTeamsO365ConnectorCardAction` | Этот метод вызывается при получении от соединителя действия Office 365 карточки соединителя. |
...
| Получены другие типы действий | `on_unrecognized_activity_type` | Этот метод можно переопределить для обработки любого типа действий, которые не обрабатываются. |

#### <a name="teams-specific-activity-handlers"></a>Обработчики действий для Teams

Расширяет `TeamsActivityHandler` список обработчиков из раздела основные обработчики Bot Framework, включив в него следующие события:
...
Список обработчиков действий Teams, вызываемых из `on_invoke_activity` обработчика действий Teams, включает следующие типы вызовов:
...
| actionableMessage/executeAction | `on_teams_o365_connector_card_action` | Этот метод вызывается при получении от соединителя действия Office 365 карточки соединителя. |
