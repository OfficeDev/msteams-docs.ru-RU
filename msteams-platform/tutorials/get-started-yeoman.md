...
---
# <a name="create-your-first-microsoft-teams-app-using-the-yeoman-generator"></a>Создайте первое Microsoft Teams приложение с помощью генератора Yeoman
...
В этом руководстве мы пройдите по созданию первого приложения Microsoft Teams с помощью Microsoft Teams генератора Yeoman. Кроме того, вы проходите процесс обновления Teams с помощью генератора Yeoman. Обязательным условием для начала с этого руководства является то, что у вас есть Teams учетная запись, которая позволяет [загрузку приложений.](~/concepts/build-and-test/prepare-your-o365-tenant.md)
...
Вкладка находится в `./src/app/scripts/myFirstAppTab/MyFirstAppTab.tsx` файле. Это класс typeScript React на основе вкладки. Найдите `render()` метод и добавьте строку кода внутри `<PanelBody>` управления, чтобы он выглядел так:
...
Сначала необходимо создать файл манифеста Teams, который вы загружаете/загружаете в Microsoft Teams. Это делается задачей Gulp `gulp manifest` . Это позволит проверить манифест и создать почтовый файл в `./package` каталоге.
...
Теперь вы должны иметь возможность просматривать, `http://localhost:3007/myFirstAppTab/` чтобы убедиться, что ваша вкладка отрисовка. Однако пока нет Microsoft Teams:
...
После запуска создайте новую команду Microsoft Teams и при ее создании щелкните имя команды, чтобы перейти к настройкам команд и выбрать `gulp ngrok-serve` *приложения.* В правом нижнем углу следует просмотреть ссылку Upload настраиваемого приложения, выберите *его,* а затем просмотрите папку проекта и подмостки с названием `package` . Выберите почтовый файл в этой папке и откройте. Ваше приложение теперь загружено в Microsoft Teams:
...
Выберите вкладку и следуйте инструкциям, чтобы добавить ее. Обратите внимание, что у вас есть настраиваемый диалог конфигурации, для которого можно изменить источник. Выберите *Сохранить,* чтобы добавить вкладку в канал. После этого вкладка должна быть загружена внутри Microsoft Teams!
